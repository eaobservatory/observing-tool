...
// 		System.out.println("Lower F = "+lowF);
// 		System.out.println("High F = "+highF);
...
//  		System.out.println("Added plot value of ("+rxPlot[index][0]+", "+rxPlot[index][1]+")");
		index++;
	    }
    ...
    /**
     * Does the actual plotting.  Overrides the paintComponent method in 
     * JPanel
     */
   public void paintComponent ( Graphics g )
   {
	 ...
	 ig = buffer.getGraphics();
	  ...
	  for ( j=0; j<rxPlot.length-1; j++) {
	      ig.drawLine ( rxPlot[j][0], rxPlot[j][1],
			    rxPlot[j+1][0], rxPlot[j+1][1] );
	  }
      }
	  ...
	  for ( j=0; j<untunablePlot.length; j++) {
	      ig.fillRect( untunablePlot[j][0], untunablePlot[j][1],
			   untunablePlot[j][2], untunablePlot[j][3]);
...
//       System.out.println("Upper limit: "+highLimit);
	  ...
	  while ( iter.hasNext() ){
	      ...
	      if (key.doubleValue() > lowLimit) break;
	  }
	  if (key == null) {
	      rxPlot = null;
	  }
	      ...
	      while ( iter.hasNext() ) {
		  key = (Double)iter.next();
		  if ( key.doubleValue() > highLimit ) break;
    ...
    /**
     * Read the receiver temperature file and get the data for the specified front end.  The
     * return is a TreeMap with frequency as the key and TRx as the value.
     */
   public TreeMap getTRx(String feName) {
	   ...
	   try {
		   ...
		   lowF  = Double.valueOf(new String(in.readLine())).doubleValue();  // Low frequency limit
		   lowF = lowF*1.0e9;
		   highF = Double.valueOf(new String(in.readLine())).doubleValue();  // High frequency limit
		   highF = highF*1.0e9;
