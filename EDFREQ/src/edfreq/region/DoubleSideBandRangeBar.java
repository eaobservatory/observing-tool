...
public class DoubleSideBandRangeBar extends JPanel implements Observer {
  ...
  private RangeBar _lowerRangeBar = new RangeBar(SIDE_BAND_LSB);
  private RangeBar _upperRangeBar = new RangeBar(SIDE_BAND_USB);
  private JLabel   _baselineId = new JLabel();

  /**
   * Width of DoubleSideBandRangeBar in GUI pixels.
   *
   * The total width represents the frequency range <tt>feBandWidth + feIF</tt>
   * where <tt>feBandWidth</tt> is the frontend bandwidth and <tt>feIF</tt>
   * is the frontend IF.
   */
  ...
  public void resetRangeBars(int feBandWidth, int totalWidth, int [] beBandRange) {
    ...
    //_beBandMin   = beBandRange[0];
    //_beBandMax   = beBandRange[1];
  ...
  public void resetRangeBars() {
    ...
    for ( int i=0; i<_xRanges.size(); i+=2 ) {
        //_lowerRangeBar.setRange(_feBandWidth - _beBandMax, _feBandWidth - _beBandMin);
        //_upperRangeBar.setRange(_beBandMin, _beBandMax);
        _lowerRangeBar.setRange(_feBandWidth - ((Integer)_xRanges.get(i+1)).intValue(), 
		_feBandWidth - ((Integer)_xRanges.get(i)).intValue(), i/2);
        _upperRangeBar.setRange( ((Integer)_xRanges.get(i)).intValue(),
		((Integer)_xRanges.get(i+1)).intValue(), i/2 );
    }
  }

  /**
   * @param feBandWidth Frontend bandwith in pixels
   * @param totalWidth    2 * (frontend IF + (0.5 * frontend bandwith)) in pixels
   * @param sideBand      SIDE_BAND_USB (upper sideband) or SIDE_BAND_LSB (lower sideband)
   * @param dsb           If true: double sideband, else single sideband
   */
  ...
  public void update(Observable o, Object arg) {
    ...
    if(source == _lowerRangeBar) {
      for ( int i=0; i<_lowerRangeBar.getNumBars(); i++ ) {
        _upperRangeBar.setBar(_upperRangeBar.getWidth() - (_lowerRangeBar.getBarX(i) + _lowerRangeBar.getBarWidth(i)),
                              _lowerRangeBar.getBarWidth(i), i);
      }
    }

    if(source == _upperRangeBar) {
      for ( int i=0; i<_upperRangeBar.getNumBars(); i++) {
        _lowerRangeBar.setBar(_lowerRangeBar.getWidth() - (_upperRangeBar.getBarX(i) + _upperRangeBar.getBarWidth(i)),
                              _upperRangeBar.getBarWidth(i), i);
      }
    }
  }
  ...
  public static void main(String [] args) {
    boolean dsb = true;
    int sideBand = SIDE_BAND_USB;

    if(args.length == 1) {
      dsb = false;

      if(args[0].equalsIgnoreCase("usb")) {
        sideBand = SIDE_BAND_USB;
      }

      if(args[0].equalsIgnoreCase("lsb")) {
        sideBand = SIDE_BAND_LSB;
      }
    }
