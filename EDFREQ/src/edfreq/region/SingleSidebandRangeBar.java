...
public class SingleSidebandRangeBar extends JPanel implements Observer {
  ...
  public void resetRangeBar() {
    ...
    for ( int i=0; i<_xRanges.size(); i+=2 ) {
        if ( _sideband == EdFreq.SIDE_BAND_LSB ) {
            _bar.setRange(_feBandWidth - ((Integer)_xRanges.get(i+1)).intValue(), 
                    _feBandWidth - ((Integer)_xRanges.get(i)).intValue(), i/2);
        }
        else {
            _bar.setRange(((Integer)_xRanges.get(i)).intValue(), 
  ...
  public void update(Observable o, Object arg) {
    ...
    if ( ((MouseEvent)arg).getID() != MouseEvent.MOUSE_RELEASED ) return;

    Object source = ((MouseEvent)arg).getSource();

    if(source == _bar) {
      for ( int i=0; i<_bar.getNumBars(); i++ ) {
        _bar.setBar(_bar.getWidth() - (_bar.getBarX(i) + _bar.getBarWidth(i)),
                              _bar.getBarWidth(i), i);
      }
    }
