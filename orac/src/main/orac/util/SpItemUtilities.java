...
public class SpItemUtilities {
    ...
    /**
     * This method calls saveElapsedTime() on all MSBs and Observations that
     * are MSBs.
     *
     * This utility method can be used to set all the elapsed times (time
     * estimates), before saving to disk or storing to database.
     * This means that this method sets the
     * {@link gemini.sp.SpMSB#ATTR_ELAPSED_TIME}
     * attributes of all SpMSB components and SpObs components that are MSBs
     * based on the current setting.  Unlike other attributes  the elapsed
     * time is not directly typed in by the user but depends on the
     * MSB/Observation as a whole. Therefore it has to be set in all SpMSB
     * components and SpObs components that are MSBs immediately
     * before saving/storing so that elapsed to written to XML is based
     * on the latest settings of these  components.
     */
     ...
     *
     * Returns an object that can be be added to an SpItem (especially
     * SpRootItem, SpProg etc) to observe hierarchy changes and execute
     * necessary code such as updating MSB attributes.
     *
    ...
    public static void setReferenceIDs(SpItem spItem) {
        ...
        if (spItem instanceof SpObsContextItem) {
            if (spItem instanceof SpMSB) {
                _insertReferenceIDsFor(
                        SpTreeMan.findTargetList(spItem), spItem);
                _insertReferenceIDsFor(
                        SpTreeMan.findInstrument(spItem), spItem);
                _insertReferenceIDsFor(
                        findSiteQuality(spItem), spItem);
                _insertReferenceIDsFor(
                        findSchedConstraint(spItem), spItem);
                _insertReferenceIDsFor(
                        SpTreeMan.findDRRecipe(spItem), spItem);
                _insertReferenceIDsFor(
                        findProgramNotes(spItem.getRootItem(),
                                new Vector<SpItem>()),
                        spItem);
                _insertReferenceIDsFor(
                        findParentNotes(spItem, new Vector<SpItem>()),
                        spItem);
     ...
     *                  This vector used to contain only items of type
     *                  SpObsComp but it can now contain any items of type
     *                  SpItem in order to allow top level SpNotes to be
     *                  inherited by MSBs that are further down in the tree.
     *                  This is the same kind of inheritance that applies to
     *                  SpObsComp items such as instrument components,
     *                  site quality components etc.
     */
    protected static void _insertReferenceIDsFor(
            Vector<SpItem> spObsCompVector, SpItem spItem) {
    ...
    /**
     * @param spObsComp This argument used to be of type SpObsComp but it has
     *                  been changed to SpItem in order to allow top level
     *                  SpNotes to be inherited by MSBs that are further down
     *                  in the tree. This is the same kind of inheritance that
     *                  applies to SpObsComp items such as instrument
     *                  components, site quality components etc.
     */
    protected static void _insertReferenceIDsFor(SpItem spObsComp,
            SpItem spItem) {
        if ((spObsComp == null) || (spItem == null)) {
            return;
        }

        // See whether the spObsComp (e.g. observation component or SpNote)
        // has an id already. If so use it for idref of the SpMSB.
        String idString = spObsComp.getTable().get(ATTR_ID);

        if (idString != null) {
            if (spObsComp.getClass().getName().indexOf("SpNote") == -1) {
                spItem.getTable().noNotifySet(
                        spObsComp.getClass().getName().substring(
                                spObsComp.getClass().getName().lastIndexOf(".")
                                        + 1)
                                + ID_REF_SUFFIX,
                        idString,
                        0);
                ...
                spItem.getTable().noNotifySet(
                        spObsComp.getClass().getName().substring(
                                spObsComp.getClass().getName().lastIndexOf(".")
                                        + 1)
                                + ID_REF_SUFFIX + idString,
                        idString,
                        0);
            }

        } else {
            // No id yet. Use _idCounter for both spObsComp (e.g. observation
            // component or SpNote) and spItem (e.g. SpMSB/SpObs).
    ...
    /**
     * Find the site quality component assoicated with this context, if any.
     *
     * Only searches the given scope.  It does not navigate the tree hierarchy.
     */
    public static SpSiteQualityObsComp findSiteQualityInContext(
            SpItem spItem) {
        SpSiteQualityObsComp sqc = null;
        SpItem returned = SpTreeMan.findSpItemInContext(spItem,
                SpSiteQualityObsComp.class);
    ...
    /**
     * Find the scheduling constraint component assoicated with this context,
     * if any.
     *
     * Only searches the given scope.  It does not navigate the tree hierarchy.
     */
    public static SpSchedConstObsComp findSchedConstraintInContext(
            SpItem spItem) {
        SpSchedConstObsComp scc = null;
        SpItem returned = SpTreeMan.findSpItemInContext(spItem,
                SpSchedConstObsComp.class);
    ...
    /**
     * Find the SpNote assoicated with this context, if any.
     *
     * Only searches the given scope.  It does not navigate the tree hierarchy.
     */
    ...
    public static SpSiteQualityObsComp findSiteQuality(SpItem spItem) {
        ...
        SpItem returned = SpTreeMan.findSpItemOfType(spItem,
                SpSiteQualityObsComp.class);
     ...
     * when the Science Program tree is disassembled into MSBs (e.g. in the
     * OMP)
     *
     * @param spItem the SpItem defining the scope to search
     */
    ...
    public static Vector<SpItem> findProgramNotes(SpItem spItem,
            Vector<SpItem> notes) {
    ...
    public static Vector<SpItem> findParentNotes(SpItem spItem,
            Vector<SpItem> notes) {
        ...
        while (!(parent instanceof SpRootItem)) {
            if (parent instanceof SpAND || parent instanceof SpOR) {
                ...
                while (e.hasMoreElements()) {
                    ...
                    if (child instanceof SpNote
                            && ((SpNote) child).isObserveInstruction()) {
                        notes.add(child);
                    }
                }
            }
