...
public final class SpInstUIST extends SpUKIRTInstObsComp {
            ...
            while ((block = instCfg.readBlock()) != null) {
                ...
                if (InstCfg.matchAttr(instInfo, "instrument_port")) {
                    ...
                    DEFAULT_PIXEL_FOV = instInfo.getValue();
                        ...
                        "default_science_area")) {
                    DEFAULT_SCIENCE_AREA = instInfo.getValue();

                } else if (InstCfg.matchAttr(instInfo,
                        "default_imaging_pos_angle")) {
                    DEFAULT_IMAGING_POS_ANGLE =
                            Double.valueOf(instInfo.getValue());
                } else if (InstCfg.matchAttr(instInfo,
                        "default_spect_pos_angle")) {
                    DEFAULT_SPECT_POS_ANGLE =
                            Double.valueOf(instInfo.getValue());
                    ...
                    DISPERSER_CHOICES_ACQ = instInfo.getValueAsArray();
                        ...
                        "disperser_choices_ifu_acq")) {
                    DISPERSER_CHOICES_IFU_ACQ = instInfo.getValueAsArray();

                } else if (InstCfg.matchAttr(instInfo,
                        "disperser_choices_pol_acq")) {
        ...
        if (sm == null) {
            if (isImaging()) {
                ...
                switch (filterSet) {
                    ...
                    default:
                        System.out.println(
                                "GetFilterLUT> Unrecognised filterSet "
                                + "number " + filterSet);
                        sm = DEFAULT_MAG1;
                }
            } else {
    ...
    public void setImager(String imager) {
        ...
        // Added by RDK 19 Nov 2003: When we change plate scale, check the
        // existing filter to make sure it exists in the filter table for
        // the new plate scale. If so, call setFilter with the current value
        // to re-set all filter-dependent values. If the current filter does
        // not exist in the filter table for the new plate scale,
    ...
    public String getDefaultFocus() {
        ...
        if (isImaging()) {
            // For imaging, focus comes from FILTERS* lut with choice depending
            // on polarimetry
    ...
    public int getMaskSet() {
        ...
        try {
            maskSet = Integer.valueOf(
                    DISPERSERS.elementAt(getDisperserIndex(), 4));
        } catch (Exception ex) {
            System.out.println("getMaskSet> failed to get maskSet"
                    + " - defaults to 1");
            maskSet = 1;
        }
    ...
    /**
     * Confirm that the current mask is available in the current mask set.
     *
    ...
    public int getBroadFilterSet() {
        ...
        if (isPupilImaging()) {
            ...
            filterSet = Integer.valueOf(
                    IMAGERS.elementAt(getImagerIndex(), 1));
        }
        // End of added by RDK
    ...
    public int getNarrowFilterSet() {
        ...
        if (isPupilImaging()) {
            ...
            filterSet = Integer.valueOf(
                    IMAGERS.elementAt(getImagerIndex(), 2));
        }
        // End of added by RDK
    ...
    public LookUpTable getFilterLUT() {
        ...
        switch (filterSet) {
            ...
            default:
                System.out.println(
                        "GetFilterLUT> Unrecognised filterSet number "
                        + filterSet);
                return FILTERS1;
        }
    }
    ...
    public String getDefaultFilter() {
        ...
        switch (filterSet) {
            ...
            default:
                System.out.println(
                        "GetDefaultFilter> Unrecognised filterSet number "
                        + filterSet);
                return DEFAULT_FILTERS1;
        }
    }
    ...
    public void setFilter(String filter) {
        ...
        try {
            ...
            if (isImaging()) {
                ...
                // Need to get the OTFilter name to lookup the focus, cwl and
                // order
            ...
            System.out.println("setFilter> Filter not recognised: "
                    + filter + " in lut");
        } catch (NoSuchElementException ex) {
            System.out.println("setFilter> Filter not recognised: "
                    + filter + " in lut");
    ...
    public LookUpTable getReadoutLUT(double et) {
        ...
        // Construct a LUT that just contains rows suitable for the exposure
        // time
        ...
        if (v.isEmpty()) {
            ...
            if (s.compareToIgnoreCase(v.firstElement()) < 0) {
                ...
                for (int i = 0; i < v.size() - 1; i++) {
                    if (s.compareToIgnoreCase(v.elementAt(i)) > 0
                            && s.compareToIgnoreCase(v.elementAt(i + 1)) < 0) {
                        v.add(i + 1, s);
                    }
                }
            }
    ...
    public String getExpTimeOTString() {
        String timeAsString =
                Double.toString(MathUtil.round(getExpTimeOT(), 4));
        return timeAsString;
    }
    ...
    public void updateDAConf(String obsType) {
        ...
        boolean ifND = W_mode.equalsIgnoreCase("NDSTARE")
                || W_mode.equalsIgnoreCase("THERMAL ND");
    ...
    // Added by RDK
    /**
     * Update the attribute-value table.
     *
     * Intended to provide backwards compatibility to make possible to
     * translate science programs created with the previous version of the OT.
     */
    public void avTableUpdate() {
        if (_avTable.get(ATTR_VERSION).equals("1")) {
            ...
            if (imager.equalsIgnoreCase("pupil")) {
                ...
                // Old OT had imager names like 0.12, 0.06+IJM, etc. We want to
                // strip off the +IJM and just leave the plate scale.
                String[] split = imager.split("+");
                _avTable.set(ATTR_IMAGER, split[0]);
            }

            // Convert readoutOT value to DAConf value by looking for a match
            // in the MODES_OT table
            ...
            if (st1.countTokens() == 2) {
                ...
                System.out.println("Unexpected value for readoutOT "
                        + readoutOT);
                setDAConf("ERROR");
            }
            ...
            // This was added to compensate for a bug in convertUISTSp.pl
            // wherein the minumum exposure time attribute was incorrectly
            // specified as DAConfMinExpTime. (RDK)
    ...
    /**
     * This method gets the acquisition time.
     *
     * It really is an estimate of how long users run movie for. For UIST,
     * we will return 5 minutes for objects fainter than 13th magnitude,
     * 1 minute otherwise (or 0 is we are not in spectroscopy mode).
     */
