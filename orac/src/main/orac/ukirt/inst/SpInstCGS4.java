...
public final class SpInstCGS4 extends SpUKIRTInstObsComp
{
	...
	private double ANGLE_OFFSET = 60. ;
	boolean centralWavelengthSet = false ;

	public static final SpType SP_TYPE = SpType.create( SpType.OBSERVATION_COMPONENT_TYPE , "inst.CGS4" , "CGS4" ) ;
	...
	public double getResolution()
	{
		...
		else
		{
			...
			if( disp.startsWith( "40" ) || disp.startsWith( "150" ) )
			{
				...
				// Echelle resolution is independent of the wavelength. Assume that the optimum order is used.
			}
			else
			{
				res = r / sw ;
	...
	{
		double cvfo = _avTable.getDouble( ATTR_CVFWAVELENGTH , 0. ) ;
		return cvfo ;
	}
	...
	public String getDefaultFilter()
	{
		...
		try
		{
			...
			// columnms because if not all orders are specified then use the _last_ as a default
		...
		{
			System.out.println( "Caught exception looking for default filter: " + ex ) ;
		}
		return filter ;
	}
	...
	public void setInstAper()
	{
		...
		double y = y0 + ROTATION_SCALE * ( Angle.cosRadians( offsetRad ) - Angle.cosRadians( offsetRad - paRad ) ) ;
