/*
 * Copyright 1999-2003 United Kingdom Astronomy Technology Centre, an
...
public class SpIterWFCAM extends SpIterConfigObsUKIRT
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "instWFCAM", "WFCAM");
    ...
    public IterConfigItem[] getAvailableItems() {
        ...
        if (instAvailable) {
            ...
            // Filters.
            iciFilter = new IterConfigItem("Filter",
                    SpInstWFCAM.ATTR_FILTER + "Iter",
                    filters);
        ...
        if (instAvailable) {
            ...
            iciReadMode = new IterConfigItem("ReadMode",
                    SpInstWFCAM.ATTR_READMODE + "Iter", readmodes);
    ...
    public Hashtable<String, String> getIterTable()
        ...
        for (int j = 0; j < iterList.size(); j++) {
            if (iterList.contains("filterIter")) {
                configTable.put("filter",
                        getConfigStep("filterIter", step));
            }

            if (iterList.contains("readModeIter")) {
                configTable.put("readMode",
                        getConfigStep("readModeIter", step));
            }

            if (iterList.contains("exposureTimeIter")) {
                configTable.put("exposureTime",
                        getConfigStep("exposureTimeIter", step));
            }

            if (iterList.contains("coaddsIter")) {
                configTable.put("coadds",
                        getConfigStep("coaddsIter", step));
            }

            if (iterList.contains("instAperXIter")) {
                configTable.put("instAperX",
                        getConfigStep("instAperXIter", step));
            }

            if (iterList.contains("instAperYIter")) {
                configTable.put("instAperY",
                        getConfigStep("instAperYIter", step));
            }

            if (iterList.contains("instAperZIter")) {
                configTable.put("instAperZ",
                        getConfigStep("instAperZIter", step));
            }

            if (iterList.contains("instAperLIter")) {
                configTable.put("instAperL",
                        getConfigStep("instAperLIter", step));
            }
        }
    ...
    public void translate(Vector<String> v)
        ...
        for (step = 0; step < nConfigs; step++) {
            ...
            try {
                ...
                throw new SpTranslationNotSupportedException(
                        "Unable to write config file for WFCAM iterator:"
                        + e.getMessage());
            }
