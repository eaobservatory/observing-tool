...
class SpIterUISTCalObsEnumeration extends SpIterEnumeration {
    ...
    protected SpIterStep _thisFirstElement() {
        ...
        _values[13] = new SpIterValue(
                SpUISTCalConstants.ATTR_CENTRAL_WAVELENGTH,
 ...
 */
@SuppressWarnings("serial")
public class SpIterUISTCalObs extends SpIterObserveBase implements
        SpTranslatable {
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "UISTCalObs", "UIST Cal Observe");
    ...
    public String getObservationTimeString() {
        double ot = Double.valueOf(
                _avTable.get(SpUISTCalConstants.ATTR_OBSERVATION_TIME));
    ...
    public void updateDAConf() {
        ...
        // The exposure time may have been over-ridden by the QT.  Only do
        // an update if it hasn't
        ...
        // The coadds may have been over-ridden by the QT.  Only do
        // an update if it hasn't
    ...
    public void translate(Vector<String> v)
        ...
        if (_avTable.exists(SpUISTCalConstants.ATTR_NREADS)) {
            configTable.put("nreads",
                    _avTable.get(SpUISTCalConstants.ATTR_NREADS));
        }

        if (_avTable.exists(SpUISTCalConstants.ATTR_CHOP_DELAY)) {
            configTable.put("chopDelay",
                    _avTable.get(SpUISTCalConstants.ATTR_CHOP_DELAY));
        }

        if (_avTable.exists(SpUISTCalConstants.ATTR_DUTY_CYCLE)) {
            configTable.put("dutyCycle",
                    _avTable.get(SpUISTCalConstants.ATTR_DUTY_CYCLE));
        }

        if (_avTable.exists(SpUISTCalConstants.ATTR_CENTRAL_WAVELENGTH)) {
            configTable.put("centralWavelength",
                    _avTable.get(SpUISTCalConstants.ATTR_CENTRAL_WAVELENGTH));
        }
        ...
        try {
            ...
            throw new SpTranslationNotSupportedException(
                    "Unable to write UISTCalObs config file");
        }
