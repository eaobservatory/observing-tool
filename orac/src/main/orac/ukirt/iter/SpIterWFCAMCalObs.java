 ...
 */
@SuppressWarnings("serial")
public class SpIterWFCAMCalObs extends SpIterObserveBase implements
        SpTranslatable {
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "WFCAMCalObs",
            "WFCAM Calibration");
    ...
    public SpIterWFCAMCalObs() {
        ...
        _avTable.noNotifySet(SpWFCAMCalConstants.ATTR_CALTYPE,
                SKYFLAT_STRING, 0);
    ...
    public double getExposureTime() {
        double et = _avTable.getDouble(SpWFCAMCalConstants.ATTR_EXPOSURE_TIME,
                0.0);
    ...
    public double getFocusTelSteps() {
        return _avTable.getDouble(FOCUS_TEL_STRING, 0.0);
    }
    ...
    public void translate(Vector<String> v)
        ...
        if (calType == FOCUS
                && _avTable.exists(SpWFCAMCalConstants.ATTR_FOCUS)) {
        ...
        configTable.put(SpWFCAMCalConstants.ATTR_EXPOSURE_TIME,
                "" + getExposureTime());
        ...
        try {
            ...
            throw new SpTranslationNotSupportedException(
                    "Unable to write WFCAM Calibration config: "
                    + ioe.getMessage());
        }
        ...
        if (calType == FOCUS
                && _avTable.getDouble(SpWFCAMCalConstants.ATTR_FOCUS, 0.0)
                        != 0.0) {
    ...
    public void processXmlElementContent(String name, String value, int pos) {
        if (name != null && name.length() > 0 && value != null) {
            if (name.equals(SpUISTCalConstants.ATTR_CALTYPE)
                    && value.equals("Focus")) {
                value = FOCUS_STRING;

            } else if (name.equals(SpUISTCalConstants.ATTR_CALTYPE)
                    && value.equals("SkyFlat")) {
                value = SKYFLAT_STRING;

            } else if (name.equals(SpUISTCalConstants.ATTR_CALTYPE)
                    && value.equals("DomeFlat")) {
