...
public class SpIterUFTI extends SpIterConfigObsUKIRT
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "instUFTI", "UFTI");
        ...
        if (attribute.equals("readoutAreaIter")) {
            ...
            if (value == null || value.equals("None")) {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            System.out.println("SpIterUFTI: Failed to find"
                                    + " filter anywhere!");
                            _avTable.set(SpInstUFTI.ATTR_INSTRUMENT_APER
                                    + "LIter", "1.0", index);
                            return;
                        }
                    }
                ...
                _avTable.set(SpInstUFTI.ATTR_INSTRUMENT_APER + "LIter",
                        farray.elementAt(filtind, 1), index);
            }
        }
    }
    ...
    public IterConfigItem[] getAvailableItems() {
        // Acquistion Mode
        IterConfigItem iciAcqMode = new IterConfigItem("Acqmode",
                SpInstUFTI.ATTR_MODE + "Iter", SpInstUFTI.MODES);

        // Readout area.  This is now a lookup table, so extract the first
        // column, and put the areas into an array.
        ...
        IterConfigItem iciReadoutArea = new IterConfigItem("ReadoutArea",
                SpInstUFTI.ATTR_READAREA + "Iter", readoutAreas);
        ...
        IterConfigItem iciFilter = new IterConfigItem("Filter",
                SpInstUFTI.ATTR_FILTER + "Iter", filters);
    ...
    public void translate(Vector<String> v)
        ...
        // First get the available items and set the defaults after making
        // sure an instrument of the right type is available
        SpInstUFTI inst;
        boolean isPol = false;
        ...
        for (int i = 0; i < nConfigs; i++) {
            ...
            try {
                ...
                throw new SpTranslationNotSupportedException(
                        "Unable to write config file for UFTI iterator:"
                        + e.getMessage());
            }

            v.add("loadConfig "
                    + ConfigWriter.getCurrentInstance().getCurrentName());
            v.add("define_inst " + getItemName() + xAper + yAper + zAper
                    + lAper);
