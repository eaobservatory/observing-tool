...
public class SpIterUISTImaging extends SpIterConfigObsUKIRT implements
        SpTranslatable {
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "instUISTImaging", "UIST Imaging");
    ...
    public IterConfigItem[] getAvailableItems() {
        ...
        if (instAvailable) {
            ...
            String filters[] = new String[filtersBroadband.length
                    + filtersNarrowband.length];
        ...
        for (int i = 0; i < nConfigs; i++) {
            ...
            for (int j = 0; j < iterList.size(); j++) {
                if (iterList.contains("filterIter")) {
                    defaultsTable.put("filter",
                            getConfigSteps("filterIter").get(i));
                }
            ...
            try {
                ...
                throw new SpTranslationNotSupportedException(
                        "Unable to write config file for UISTImaging iterator:"
                        + e.getMessage());
            }
