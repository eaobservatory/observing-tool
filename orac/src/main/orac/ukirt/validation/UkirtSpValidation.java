 ...
 *
 * The class contains a main method and can be used as a stand alone
 * command line tool to validate science programs.
 *
...
public class UkirtSpValidation extends SpValidation {
    ...
    /**
     * Indicates whether the current is validation is a science program check
     * or an observation check.
     *
     * If this is set to true then the method checkObservation "knows" that
     * it only has to check whether there is a target list. If the target list
     * is inside the observation its values will be checked in checkContextItem
     * (since an observation is also a context item). If the observation
     * inherits its target list from the science program then the target list
     * values will be checked in checkObservation only if the current check
     * is an observation check (i.e. _isSpProgCheck is false). If the current
     * check is a science program check (i.e. _isSpProgCheck is true) then
     * the target list check is done once in checkSciProgram and not repeated
     * in all the observation checks.
     *
     ...
     * NEVER USED.
     *
     * This reset method is not as "important" as, say, CGS4Validation#reset
     * because none of the values that are reset is static and the values are
     * all proberly initialised for each new UkirtSpValidation object.
     */
    public void reset() {
    }
     ...
     */
    public void findSpItemByClassName(SpItem spItem, String type,
            String tree_path) {
        SpItem child = null;
    ...
    /**
     * Returns true if SpItem of specified type and subtype is found in the
     * subtree below SpItem argument.
     *
     * Uses the type and subtype strings defined in gemini.sp.SpType.
     * Returns immediately once a match is found without checking whether
    ...
    public boolean findSpItem(SpItem spItem, String type, String subtype) {
        ...
        if ((System.getProperty("DEBUG") != null)
            ...
            debug = true;
            System.out.println("examining " + spItem.name() + ": "
                    + spItem.typeStr() + " (" + spItem.subtypeStr() + "), "
                    + spItem.getClass().getName());
        }

        if (spItem.typeStr().equals(type)
                && spItem.subtypeStr().equals(subtype)) {
            if (debug) {
     ...
     *
     * It should be called from with in another method that checks all
     * SpItem recursively.
     */
    public void checkInstrumentComponents(SpInstObsComp spInstObsComp,
        ...
        if (spInstObsComp instanceof orac.ukirt.inst.SpInstMichelle) {
            ...
            System.out.println("Unexpected instrument component: "
                    + spInstObsComp.subtypeStr());
        }
    }
     ...
     *
     */
    public void checkInstrumentIterators(Vector<SpItem> iterators,
            Vector<ErrorMessage> report) {
        if (iterators == null) {
            return;
        }
        ...
        /*
         * Checking for valid instrument component / instrument iterator
         * combination.  First check whether the iterator is instrument
         * specific.  This assumes that subtypes of instrument iterators
         * start with "inst" (and NOT with "inst.")
         */
        SpIterConfigObs currentIterator = null;
        SpInstObsComp currentInstObsComp = null;
        String currentInstrument = null;
        // iteratorInstrument is not an instrument iterator but the name of
        // the instrument of an instrument iterator.
        String iteratorInstrument = null;

        for (Enumeration<SpItem> e = iterators.elements();
                e.hasMoreElements();) {
            ...
            if (currentInstObsComp != null) {
                currentInstrument =
                    currentInstObsComp.subtypeStr().substring(5);
                iteratorInstrument = currentIterator.subtypeStr().substring(4);

                for (int j = 0; j < instruments.length; j++) {
                    if (instruments[j].compareToIgnoreCase(iteratorInstrument)
                            == 0) {
                        // If the iterator is instrument specific then it
                        // should be for the current instrument.
                        if ((currentInstrument != null)
                                && (currentInstrument.compareToIgnoreCase(
                                        iteratorInstrument) != 0)) {
                            report.add(new ErrorMessage(ErrorMessage.ERROR,
                                    iteratorInstrument + " iterator",
                                    "You cannot use a " + iteratorInstrument
                                            + " iterator in combination with "
                                            + currentInstrument + "."));
                        }
                    }
                }

                // Checking for invalid instrument component / FP iterator
                // combinations.
                if (currentIterator instanceof SpIterFP) {
                    if (currentInstObsComp instanceof SpInstCGS4
                        ...
                        report.add(new ErrorMessage(ErrorMessage.ERROR,
                                "FP iterator",
                                "You cannot use an FP iterator in combination"
                                        + " with the instrument "
                                        + currentInstrument + "."));
                    }
                }
                ...
                if (currentIterator instanceof SpIterIRPOL) {
                    Vector<SpItem> irPolChildren = findInstances(
                            (SpIterIRPOL) currentIterator, SpIterIRPOL.class);

                    // There should be only 1 element in the array; the
                    // current iterator
                    if (irPolChildren != null && irPolChildren.size() > 1) {
                        report.add(new ErrorMessage(ErrorMessage.ERROR,
                                "IRPOL iterator",
                                "An IRPOL iterator should not contain another"
                                        + " IRPOL iterator"));
                    }
                }
            }
    ...
    public void checkObservation(SpObs spObs, Vector<ErrorMessage> report) {
        ...
        // Check whether the observation has an instrument (as its own child OR
        // in its context).
        if (SpTreeMan.findInstrument(spObs) != null && !_isSpProgCheck) {
            checkInstrumentComponents(SpTreeMan.findInstrument(spObs), report);
        }

        // Check whether the observation has a target list (as its own child OR
        // in its context).  Also need to check if this observation has a
        // survey container as a parent.
        ...
        // Check whether the observation a DR recipe (as its own child OR
        // in its context).
        ...
        observeIterators = findInstances(spObs,
                "gemini.sp.iter.SpIterObserveBase");
        ...
        observeIterators = findInstances(spObs,
                "orac.ukirt.iter.SpIterDarkObs");

        // Now find instrument iterators.
        instrumentIterators = findInstances(spObs,
                "gemini.sp.iter.SpIterConfigObs");

        if (instrumentIterators == null) {
            if (SpTreeMan.findInstrument(spObs) == null) {
                report.add(new ErrorMessage(ErrorMessage.ERROR, titleString,
                        "Neither instrument iterator nor"
                        + " instrument component found."));
            }
        } else {
        ...
        if (observeIterators != null) {
            ...
            for (Enumeration<SpItem> e1 = observeIterators.elements();
                    e1.hasMoreElements();) {
                darkExpTime = ((SpIterObserveBase)
                        e1.nextElement()).getExposureTime();

                if (darkExpTime != instExpTime) {
                    report.add(new ErrorMessage(ErrorMessage.WARNING,
                            titleString,
                            "DARK doesn't have same exposure time"
                            + " as instrument component."));
                }

                if (instrumentIterators != null) {
                    for (Enumeration<SpItem> e2 =
                            instrumentIterators.elements();
                            e2.hasMoreElements();) {
                        tmpInstIter = (SpIterConfigObs) e2.nextElement();

                        if (tmpInstIter != null
                                && tmpInstIter.getExposureTimes() != null) {
                            for (Enumeration<?> e3 =
                                    tmpInstIter.getExposureTimes().elements();
                                    e3.hasMoreElements();) {
                                try {
                                    instIterExpTime =
                                ...
                                if (darkExpTime != instIterExpTime) {
                                    report.add(new ErrorMessage(
                                            ...
                                            "DARK doesn't have same exposure"
                                            + " time as instrument iterator."
                                            ));
                                }
                            }
                        }
        ...
        /*
         * Checking that there is no offset followed by an instrument in the
         * sequence and whether an SpIterObserve in the sequence is followed
         * by an instrument iterator or an offset iterator.
         */
        ...
        Vector<SpItem> iterFolderVector = findInstances(spObs,
                "gemini.sp.iter.SpIterFolder");

        if (iterFolderVector != null) {
            for (Enumeration<SpItem> e1 = iterFolderVector.elements();
                    e1.hasMoreElements();) {
                iterFolder = (SpIterFolder) e1.nextElement();

                for (Enumeration<SpItem> e2 = iterFolder.children();
                        e2.hasMoreElements();) {
                    previousItem = currentItem;
                    currentItem = e2.nextElement();

                    if ((previousItem instanceof SpIterOffset)
                            && (currentItem instanceof SpIterConfigObs)) {
                        report.add(new ErrorMessage(ErrorMessage.WARNING,
                                "Iterator Folder in " + titleString,
                                "There is an offset iterator followed by"
                                + " an instrument iterator."));
                    }
                    ...
                    if (spIterObserveFound
                        ...
                        report.add(new ErrorMessage(ErrorMessage.ERROR,
                                "Iterator Folder in " + titleString,
                                "Instrument iterators are not allowed"
                                + " after the observe."));
                    }
                ...
                if ((instObsComp instanceof SpInstUFTI)
                        || (instObsComp instanceof SpInstIRCAM3)) {
                    if (!dark) {
                        report.add(new ErrorMessage(ErrorMessage.WARNING,
                                titleString,
        ...
        /*
         * check for position iterators or instrument iterators that are on
         * the same level of iteration as the OBSERVE observe iterator.
         */
        observeIterators = findInstances(spObs,
                "orac.ukirt.iter.SpIterObserve");
        SpIterObserve iterObserve = null;
        SpItem sibling = null;
        boolean wrongIteratorsOnObserveLevel = false;

        if (observeIterators != null) {
            for (Enumeration<SpItem> e = observeIterators.elements();
                    e.hasMoreElements();) {
                iterObserve = (SpIterObserve) e.nextElement();

                sibling = iterObserve.next();
                while ((sibling != null) && (!wrongIteratorsOnObserveLevel)) {
                    if ((sibling instanceof SpIterConfigObs)
                            || (sibling instanceof SpIterOffset)) {
                ...
                if (wrongIteratorsOnObserveLevel) {
                    report.add(new ErrorMessage(
                            ...
                            titleString,
                            "The position iterator or instrument iterator"
                            + " and the \"observe\" are on the same"
                            + " level of iteration."));
                }
            }
        }
     ...
     * @param report              error messages and warnings are appended to
     *                            report.
     */
    public void checkTargetList(SpTelescopeObsComp telescopeObsComp,
            Vector<ErrorMessage> report) {
        ...
        for (int i = 0; i < position.length; i++) {
            ...
            try {
                trackingSystem = telescopeObsComp.getTable().getAll(
                        pos.getTag()).get(4);
            } catch (Exception e) {
                // catching possible NullPointerException or
             ...
             * Note that getXaxis() maps values in the range 0..24 h typed
             * by the user to degrees 0..360.  It also works modulo 24.
             * So 12:00:00.0 and 36:00:00.0 both yield 180.0.
             * getXaxis also treats things like 9h 16min as equivalent to
             * 8h 76min etc.
             *
             * But the user should not be allowed to type 36h instead of 12h
             * etc so checking is based on the original string.
             */
            if ((pos.getSystemType() == SpTelescopePos.SYSTEM_SPHERICAL)
                ...
                if (Xaxis == 0 && Yaxis == 0) {
                    report.add(new ErrorMessage(ErrorMessage.WARNING,
                            "Telescope target " + pos.getName() + titleString,
                ...
                if (!HHMMSS.validate(hhmmss)) {
                    report.add(new ErrorMessage(ErrorMessage.ERROR,
                            "Telescope target " + pos.getName() + titleString,
                            "RA" + converted, "range 0:00:00 .. 24:00:00",
                ...
                if (!DDMMSS.validate(ddmmss, -40, 60)) {
                    report.add(new ErrorMessage(ErrorMessage.ERROR,
                            "Telescope target " + pos.getName() + titleString,
                            "Dec" + converted, "range -40:00:00 .. 60:00:00",
                            pos.getYaxisAsString()));
                }
            }

            /*
             * Check whether the target list has a target coordinate for which
             * the epoch is B1950, and a guide star coordinate for which the
             * RA and Dec values are identical and the epoch is J2000.
             */
            for (int j = 0; j < position.length; j++) {
                ...
                if (i != j) {
                    if ((pos.getXaxis() == pos2.getXaxis())
                        ...
                        try {
                            trackingSystem2 =
                                    telescopeObsComp.getTable().getAll(
                                            pos2.getTag()).get(4);
                        } catch (Exception e) {
                            // catching possible NullPointerException or
                            // ArrayIndexOutOfBoundsException
                        ...
                        boolean trackingGood = trackingSystem != null
                                && trackingSystem2 != null;
                        boolean guidesGood = pos.isGuidePosition()
                                && !pos2.isGuidePosition();
                        boolean systemsDiffer =
                                trackingSystem != trackingSystem2;

                        if (trackingGood && guidesGood && systemsDiffer) {
                            report.add(new ErrorMessage(
                                    ErrorMessage.ERROR,
                                    telescopeObsComp.getTitle() + titleString,
                                    "Target and guide star with differing"
                                    + " coordinate systems have identical"
                                    + " values for Ra and Dec."));
                        }
                    }
                }
            }
        ...
        if (hasGuideStar) {
            ...
            SpTelescopePos guidePos =
                    (SpTelescopePos) list.getPosition("GUIDE");
            ...
            if (basePos != null && guidePos != null) {
                ...
                // Guides are never offsets for UKIRT, so we need to work out
                // the offsets
                double[] offsets = RADecMath.getOffset(guideRA, guideDec,
                        baseRA, baseDec);
                for (int i = 0; i < offsets.length; i++) {
                    if (Math.abs(offsets[i]) > 210) {
                        report.add(new ErrorMessage(ErrorMessage.ERROR,
                                basePos.getName() + titleString,
                                "Guide star is more than 210 arcsecs"
                                + " from the base"));
                        break;
                    }
                }
    ...
    public void checkDRRecipe(SpDRRecipe recipe, Vector<ErrorMessage> report) {
        ...
        if (inst == null) {
            ...
            // NOTE that _IN_GROUP_ recipes are not considered because they
            // are currently all set to false in SpDRRecipe.
            boolean flat = false;
            boolean arc = false;
            boolean bias = false;
            boolean focus = false;
            ...
            if (inst instanceof SpInstUFTI) {
                ...
                System.out.println("No dr recipes for instrument "
                        + inst.subtypeStr());
            }
            ...
            if (darkSkyAndObject) {
                if (!recipes.contains(recipe.getDarkRecipeName())) {
                    report.add(new ErrorMessage(ErrorMessage.WARNING,
                            "DR Recipe (Dark) for " + inst.subtypeStr()
                                    + titleString,
                            recipe.getDarkRecipeName()
                                    + " not in the OT list."));
                }

                if (!recipes.contains(recipe.getSkyRecipeName())) {
                    report.add(new ErrorMessage(ErrorMessage.WARNING,
                            "DR Recipe (Sky) for " + inst.subtypeStr()
                                    + titleString,
                            recipe.getSkyRecipeName()
                                    + " not in the OT list."));
                }

                if (!recipes.contains(recipe.getObjectRecipeName())) {
                    report.add(new ErrorMessage(ErrorMessage.WARNING,
                            "DR Recipe (Object) for " + inst.subtypeStr()
                                    + titleString,
                            recipe.getObjectRecipeName()
                                    + " not in the OT list."));
                }
            }
            ...
            if (flat && !recipes.contains(recipe.getFlatRecipeName())) {
                report.add(new ErrorMessage(ErrorMessage.WARNING,
                        "DR Recipe (Flat) for " + inst.subtypeStr()
                                + titleString,
            ...
            if (arc && !recipes.contains(recipe.getArcRecipeName())) {
                report.add(new ErrorMessage(ErrorMessage.WARNING,
                        "DR Recipe (Arc) for " + inst.subtypeStr()
                                + titleString,
            ...
            if (bias && !recipes.contains(recipe.getBiasRecipeName())) {
                report.add(new ErrorMessage(ErrorMessage.WARNING,
                        "DR Recipe (Bias) for " + inst.subtypeStr()
                                + titleString,
                        recipe.getBiasRecipeName()
                                + " not in the OT list."));
            }

            if (focus && !recipes.contains(recipe.getFocusRecipeName())) {
                report.add(new ErrorMessage(ErrorMessage.WARNING,
                        "DR Recipe (Focus) for " + inst.subtypeStr()
                                + titleString,
                        recipe.getFocusRecipeName()
                                + " not in the OT list."));
            }
        }
    }
     ...
     *
     * checkSciProgram is not just a top level class applied to SpProg objects.
     * It is applied recursively to an spItem's children until a target list
     * object or an observation object is found and checkTargetList or
    ...
    public void checkSciProgram(SpProg spProg, Vector<ErrorMessage> report) {
        ...
        if (SpTreeMan.findAllItems(spProg, SpObs.class.getName()).size() < 1) {
            report.add(new ErrorMessage(ErrorMessage.WARNING,
                    spProg.getTitle(), "No observation."));
        }

        /*
         * Check context items (e.g folders, groups, observations)
         * Note that observations are checked twice: as context items AND in
         * checkObservation for where things specific to observations are
         * checked.
         *
        ...
        if (instances != null) {
            for (Enumeration<SpItem> e = instances.elements();
                    e.hasMoreElements() && !standardObsFound;) {
        ...
        /*
         * If there are observations that use CGS4 check whether there is a
         * CGS4 calibration observation i.e. an observation containing both
         * ARC and FLAT
         */
        if (instances != null && instances.size() > 0) {
            if (findCGS4CalObservation(spProg) == null) {
                report.add(new ErrorMessage(
                        ErrorMessage.WARNING,
                        spProg.getTitle() + ", CGS4",
                        "There are one or more observations that use CGS4"
                        + " but there is no observation that contains"
                        + " a FLAT and an ARC."));
            }
        }
     ...
     *
     * In the case of SpProg check is called from within checkSciProgram
     * which contains addtional checks.
     *
     * The method is NOT called recursively!!!
     */
    public void checkContextItem(SpObsContextItem contextItem,
            Vector<ErrorMessage> report) {
        ...
        while (children.hasMoreElements()) {
            ...
            /*
             * This assumes that the instrument component appears above the
             * instrument iterator in the tree.  And it assumes that subtypes
             * of instrument component start with "inst." (and NOT with "inst").
             * currentInstrument = spItem.subtypeStr().substring(5);
             */

            if (spItem instanceof SpIterFolder
                    && !(contextItem instanceof SpObs)) {
                report.add(new ErrorMessage(ErrorMessage.WARNING,
                        "Iterator Folder in " + contextItem.getTitle(),
                        "Iterator folders should only be used inside"
                                + " an observation."));
     ...
     *
     * Similar to gemini.sp.SpTreeMan.findAllItems but
     * gemini.sp.SpTreeMan.findAllItems returns only exact matches.
     * This method returns spItems whose class is a subtype of that specified
     * by classname too.
     *
     * @param result the instances are appended to the result vector. So
     *               findInstances can be called several times with the same
     *               vector so that instances can be accumulated.
     *
     * @see gemini.sp.SpTreeMan#findAllItems
     */
    public static void findInstances(SpItem spItem, Class<?> c,
            Vector<SpItem> result) {
        if (result == null) {
    ...
    public static void findInstances(SpItem spItem, String className,
            Vector<SpItem> result) {
        try {
    ...
    public static Vector<SpItem> findInstances(SpItem spItem,
            String className) {
    ...
    /**
     * This method looks for observations whose iterator folder contains at
     * least 2 observe iterators DARK and no other observe iterators.
     *
    ...
    public static Vector<SpObs> findArrayTests(SpProg spProg) {
        ...
        if (observations != null) {
            ...
            for (Enumeration<SpItem> e = observations.elements();
                    e.hasMoreElements();) {
                spObs = (SpObs) e.nextElement();
                observeIterators = findInstances(spObs,
                        "gemini.sp.iter.SpIterObserveBase");
                darkIterators = findInstances(spObs,
                        "orac.ukirt.iter.SpIterDarkObs");

                // check that there are at least two dark iterators.
                if (darkIterators != null && darkIterators.size() >= 2) {
                    // check whether that there are no observe iterators other
                    // than dark iterators.
                    if (observeIterators != null
                            && observeIterators.size() == darkIterators.size()) {
                        result.add(spObs);
                    }
                }
            }
    ...
    /**
     * This method looks for observations whose iterator folder contains at
     * least one CGS4 calibration observe iterator
     *
    ...
    public static Vector<SpObs> findCGS4CalObservation(SpProg spProg) {
        ...
        if (observations != null) {
            ...
            for (Enumeration<SpItem> e = observations.elements();
                    e.hasMoreElements();) {
                spObs = (SpObs) e.nextElement();
                cgs4CalObsIterators = findInstances(spObs,
                        "orac.ukirt.iter.SpIterCGS4CalObs");
                ...
                if (cgs4CalObsIterators != null) {
                    for (Enumeration<SpItem> e2 =
                            cgs4CalObsIterators.elements();
                            e2.hasMoreElements();) {
                        if (((SpIterCGS4CalObs) e2.nextElement()).getCalType()
                                == SpIterCGS4CalObs.ARC) {
                            arc = true;
                        } else {
                            flat = true;
                        }
    ...
    public void print(SpItem spItem, String indentStr) {
        String t = spItem.name() + ": " + spItem.typeStr() + " ("
                + spItem.subtypeStr() + "), " + spItem.getClass().getName();
        ...
        while (keys.hasMoreElements()) {
            ...
            System.out.println(indentStr + key + " ("
                    + spItem.getTable().getDescription(key) + ")");
            System.out.println(indentStr + "--> "
                    + spItem.getTable().getAll(key).toString());
        }

        // Print the children
        for (SpItem child = spItem.child(); child != null;
                child = child.next()) {
    ...
    public void checkSchedElevationRange(SpSchedConstObsComp schedule,
            Vector<ErrorMessage> report) {
        ...
        try {
            ...
            if (schedule.getDisplayAirmass()) {
                ...
                if (maxEl < 1.0) {
                    report.add(new ErrorMessage(
                            ErrorMessage.ERROR,
                            "Min elevation as airmass in schedule constraint"
                            + " is less than 1.",
                            ""));
                }
                ...
                if (minEl < 30.0) {
                    report.add(new ErrorMessage(
                            ErrorMessage.ERROR,
                            "Min elevation in schedule constraint"
                            + " is less than 30.",
                            ""));
                }
                ...
                if (maxEl > 90.0) {
                    report.add(new ErrorMessage(
                            ErrorMessage.ERROR,
                            "Max elevation in schedule constraint"
                            + " is greater than 90.",
                            ""));
                }
            }
            ...
            report.add(new ErrorMessage(ErrorMessage.ERROR,
                    "No value set for elevation in schedule constraint.", ""));
        }
    }
}
