...
public class Scuba2Time implements SpJCMTConstants {
    ...
    double stepsBetweenDarks = 1440000.0;
    ...
    public double scan(SpIterRasterObs raster) {
        ...
        if (SCAN_PATTERN_LISSAJOUS.equals(pattern)
                || SCAN_PATTERN_PONG.equals(pattern)) {
            ComputePong computePong = new ComputePong();
            String type = ComputePong.PongScan.CURVY;
            durationPerArea = computePong.getPeriodForPong(height, width, dy,
                    velocity, type);
            ...
            throw new RuntimeException("Unrecognised scan pattern: "
                    + pattern);
        }

        // System.out.println( "Estimated time to cover the map area once: "
        //         + durationPerArea + " sec" );
        ...
        if (sampleTime == 0) {
            ...
            // System.out.println("Total duration requested for scan map: "
            //         + sampleTime + " secs.");
        } else {
            int nRepeats = 1;
            ...
            // System.out.println("Number of repeats of map area requested: "
            //         + nRepeats);
        ...
        // System.out.println("Number of steps in scan map sequence: "
        //         + josMin);
    ...
    public double fts2(SpIterFTS2Obs fts2) {
        ...
        if (airmass > -1.0) {
            ...
            if (siteQuality != null) {
                ...
                if (fourFifty) {
                    NEFD = s2n.calculateNEFD(Scuba2Noise.four50, csoTau,
                            airmass);
                } else {
                    NEFD = s2n.calculateNEFD(Scuba2Noise.eight50, csoTau,
                            airmass);
                }
            }
        }
        ...
        double sensitivity = 0.0; // in mJy Hz ^ -0.5
        ...
        integrationTime = (multiplyMapArea / FOVEfficiancy)
                * (NEFD / resolutionSensitivity);

        return integrationTime;
    }

    public double ftsSensitivity(SpIterFTS2Obs fts2, double integrationTime,
            String fourFiftyOr850) {
        double multiplicand = 0.0;
        double FOV = fts2.getFOV();
        FOV *= FOV; // in square arcminutes
        double mapArea = FOV; // in square degrees, for now assume FOV
        ...
        double sensitivity = (multiplyMapArea / FOVEfficiancy)
                * (NEFD / resolutionSensitivity);
    ...
    private double focus(SpIterFocusObs focus) {
        return focus.getFocusPoints() * 60.0;
    }
