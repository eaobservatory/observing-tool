...
public class ComputePong {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1 && "-test".equals(args[0])) {
            ...
            System.out.println("Arguments are: height width spacing velocity"
                    + " type[CURVY|ROUNDED|SQUARE|STRAIGHT]");
            System.exit(-1);
        }
        ...
        double period = computePong.getPeriodForPong(height, width, spacing,
                velocity, type);
     ...
     */
    public double getPeriodForPong(double height, double width, double spacing,
            double velocity, String type) {
    ...
    public PongScan computePongGrid(PongScan pongScan) {
        ...
        double vertSpacing = (2.0 * pongScan.spacing()) / sqrt2;
        ...
        */
         ...
         */
        double gridSpace = vertSpacing / 2.0;
        ...
        pongScan.setAlphaX(xNumVert * vertSpacing / 2.0);
        pongScan.setAlphaY(yNumVert * vertSpacing / 2.0);
        double v2 = pongScan.velocity() * sqrt2 / 2.0;
        pongScan.setBetaX(xNumVert * vertSpacing * 2.0 / v2);
        pongScan.setBetaY(yNumVert * vertSpacing * 2.0 / v2);

        /*
         * Compute the time required to complete
         * one circuit of the CURVY_PONG path
         */
        pongScan.setPeriod(xNumVert * yNumVert * vertSpacing * 2.0 / v2);
    ...
    public PongScan computePongPath(PongScan pongScan) {
        // 45 deg arc at start and end of straight-line segment for
        // ROUNDED_PONG
        final double ARC_LENGTH_NOM = 45.0;
        ...
        double curveRadius = pongScan.spacing() / 2.0;
        ...
        for (int i = 0; i < nSeg; i++) {
            double xStartArcCenter = 0.0;
            double yStartArcCenter = 0.0;
            double startThetaInit = 0.0;
            double startThetaFin = 0.0;
            double startRotDir = 0.0;
            double xEndArcCenter = 0.0;
            double yEndArcCenter = 0.0;
            double endThetaInit = 0.0;
            double endThetaFin = 0.0;
            double endRotDir = 0.0;
            ...
            */
            if (PongScan.SQUARE.equals(pongScan.type())) {
                ...
                /*
                * For ROUNDED_PONG, the actual start and end points are
                * computed from the nominal start and end points, modified by
                * the arcs placed at the start and end to round off the sharp
                * corners.
                */
                ...
                * start and end arcs into a single arc of twice the nominal
                * arc length
                */
                ...
                */
                ...
                if (Math.abs(xEndNom - posXBoundary) < EPSILON) {
                    ...
                    endRotDir = (dyNom > 0) ? 1.0 : -1.0;
                    endThetaInit = 0.0 - endRotDir * arcLength;
                } else if (Math.abs(xEndNom - negXBoundary) < EPSILON) {
                    xEndArcCenter = xEndNom + curveRadius * sqrt2;
                    yEndArcCenter = yEndNom;
                    endRotDir = (dyNom > 0) ? -1.0 : 1.0;
                    endThetaInit = 180.0 - endRotDir * arcLength;
                } else if (Math.abs(yEndNom - posYBoundary) < EPSILON) {
                    xEndArcCenter = xEndNom;
                    yEndArcCenter = yEndNom - curveRadius * sqrt2;
                    endRotDir = (dxNom > 0) ? -1.0 : 1.0;
                    endThetaInit = 90.0 - endRotDir * arcLength;
                } else if (Math.abs(yEndNom - negYBoundary) < EPSILON) {
                    xEndArcCenter = xEndNom;
                    yEndArcCenter = yEndNom + curveRadius * sqrt2;
                    endRotDir = (dxNom > 0) ? 1.0 : -1.0;
                    endThetaInit = 270.0 - endRotDir * arcLength;
                } else {
                    // status = PTCS__GERROR
                }
            ...
            if (PongScan.ROUNDED.equals(pongScan.type())) {
                ...
                segLength = D2PI * curveRadius * arcLength / 360.0;
                pathLength += segLength;

                // TSTART for path segments after the first is the TEND from
                // the previous segment
                if (pathSegmentIndex > 0) {
                    segment.setTStart(pathData[pathSegmentIndex - 1].tEnd());
                }

                segment.setTEnd(segment.tStart()
                        + segLength / pongScan.velocity());
            }
            ...
            */
            if (PongScan.ROUNDED.equals(pongScan.type()) && length > EPSILON) {
                ...
                pathLength += segLength;
                segment.setTStart(pathData[pathSegmentIndex - 1].tEnd());
                segment.setTEnd(segment.tStart() + segLength
                        / pongScan.velocity());
            }
        }
    ...
    public class PongScan {
        ...
        public int xNGridSeg() {
            return xNGridSeg;
        }

        public int yNGridSeg() {
            return yNGridSeg;
        }
        ...
        public void setGridSpace(double value) {
            gridSpace = value;
        }

        public void setXNGridSeg(int value) {
            xNGridSeg = value;
        }

        public void setYNGridSeg(int value) {
            yNGridSeg = value;
        }

        public void setXVertex(double[] value) {
            xVertex = value;
        }

        public void setYVertex(double[] value) {
            yVertex = value;
        }

        public void setAlphaX(double value) {
            alphaX = value;
        }

        public void setAlphaY(double value) {
            alphaY = value;
        }

        public void setBetaX(double value) {
            betaX = value;
        }

        public void setBetaY(double value) {
            betaY = value;
        }
        ...
        public void setNumPathSegments(int value) {
            numPathSegments = value;
        }
        ...
        public void setPathData(PongSegment[] value) {
            pathData = value;
        }
    }
    ...
    private class PongSegment {
        ...
        public PongArcSegment arc() {
            return arc;
        }

        public PongStraightSegment straight() {
            return straight;
        }
        ...
        public void setArc(PongArcSegment value) {
            arc = value;
        }

        public void setStraight(PongStraightSegment value) {
            straight = value;
        }
    ...
    private class PongArcSegment {
        ...
        public double thetaInit() {
            return thetaInit;
        }

        public double thetaFin() {
            return thetaFin;
        }
        ...
        public void setXCenter(double value) {
            xCenter = value;
        }

        public void setYCenter(double value) {
            yCenter = value;
        }
        ...
        public void setThetaInit(double value) {
            thetaInit = value;
        }

        public void setThetaFin(double value) {
            thetaFin = value;
        }

        public void setRotDir(double value) {
            rotDir = value;
        }
    }
