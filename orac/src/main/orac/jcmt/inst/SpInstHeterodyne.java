...
public class SpInstHeterodyne extends SpJCMTInstObsComp {
    ...
    public String toConfigXML(String indent) {
        ...
        xmlBuffer.append(
                ...
                + indent + "    <if_centre_freq units=\"GHz\" value=\""
                    + getFeIF() + "\"/>\n"
                + indent + "    <sideband value=\"" + sideband + "\"/>\n"
                + indent + "    <sb_mode value=\""
                    + getMode().toUpperCase() + "\"/>\n"
        ...
        if (getFrontEnd().equals("HARP")) {
            xmlBuffer.append(indent
                    + "    <channel_mask>\n"
                    // Array of (OFF | ON | NEED). Use Pixeltool to switch
                    // pixels ON/OFF. NEED???
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"00\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"01\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"02\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"03\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"04\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"05\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"06\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"07\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"08\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"09\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"10\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"11\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"12\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"13\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"14\" VALUE=\"ON\"/>\n"
                    + indent
                    + "      <CHAN_MASK_VALUE CHAN=\"15\" VALUE=\"ON\"/>\n"
                    + indent + "    </channel_mask>\n");
        }
        ...
        for (int i = 0; i < getNumSubSystems(); i++) {
            ...
            xmlBuffer.append(indent + "    <rest_frequency id=\""
                    + restFreqRefs[i] + "\" units=\"GHz\">"
        ...
        xmlBuffer
                .append(indent
                        + "  <acsis_spw_list>\n"
                        + indent
                        + "    <doppler_field ref=\"TCS.RV.DOPPLER???\"/>\n"
                        + indent
                        + "    <spectral_window_id_field ref=\"SPECTRAL_WINDOW_ID???\"/>\n"
                        + indent
                        + "    <front_end_lo_freq_field ref=\"FE.STATE.LO_FREQ\"/>\n");

        // Spectral windows
        for (int i = 0; i < getNumSubSystems(); i++) {
            xmlBuffer.append(spectralWindowToXML(restFreqRefs[i], sideband,
                    "  <!-- <base_line_fit> etc. not implemented yet. -->",
                    indent + "    ", i)
                    + "\n");
        }

        xmlBuffer.append(indent + "  </acsis_spw_list>\n" + indent + "</"
                + XML_ELEMENT_ACSIS_CONFIGURATION + ">\n");

        return xmlBuffer.toString();
    }

    public String spectralWindowToXML(String restFrequencyId, int sideband,
            String dataReductionXML, String indent, int subsystem) {
        return indent + "<spectral_window id=\"SPW" + (subsystem + 1) + "\">\n"
                ...
                + indent + "    <spw_reference_pixel>" + "4064.0???"
                    + "</spw_reference_pixel>\n"
                + indent + "    <spw_if_channel_width units=\"Hz\">"
                    + getBandWidth(subsystem) + "</spw_if_channel_width>\n"
                + indent + "    <spw_number_if_channel>"
                    + getChannels(subsystem) + "</spw_number_if_channel>\n"
                + indent + "  </spw_if_coordinate>\n"
