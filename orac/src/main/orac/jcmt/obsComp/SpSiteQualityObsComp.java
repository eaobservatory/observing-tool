...
public class SpSiteQualityObsComp extends
        gemini.sp.obsComp.SpSiteQualityObsComp {
    ...
    public static final String ATTR_NOISE_CALCULATION_TAU =
            "noiseCalculationTau";
     ...
     *
     * @see #processXmlElementContent(java.lang.String,java.lang.String)
     */
    private String _previousXmlElement = "";

    // Register the prototype. This replaces
    // gemini.sp.obsComp.SpSiteQualityObsComp with
    // orac.jcmt.obsComp.SpSiteQualityObsComp
    static {
    ...
    protected void processAvAttribute(String avAttr, String indent,
        ...
        if (avAttr.equals(ATTR_SEEING)) {
            // if getSeeing() == SEEING_ANY then the option "Don't Care" has
            // been chosen and no XML should be written for seeing.
            if (getSeeing() < SEEING_ANY) {
                xmlBuffer.append("\n  " + indent + "<" + XML_SEEING + ">");
                xmlBuffer.append("\n    " + indent
                        + "<" + XML_MIN + ">" + SEEING_RANGES[getSeeing()][0]
                        + "</" + XML_MIN + ">");
                xmlBuffer.append("\n    " + indent
                        + "<" + XML_MAX + ">" + SEEING_RANGES[getSeeing()][1]
                        + "</" + XML_MAX + ">");
                xmlBuffer.append("\n  " + indent + "</" + XML_SEEING + ">");
            }
        ...
        } else if (avAttr.equals(ATTR_MIN_TAU)
                || (avAttr.equals(ATTR_MAX_TAU) || avAttr
                        .equals(ATTR_TAU_BAND_ALLOCATED))) {
            // Ignore. Has been processed with ATTR_TAU_BAND_ALLOCATED
            // attribute.
    ...
    public void processXmlElementContent(String name, String value) {
        if (name.equals(XML_SEEING)) {
            ...
            if (_previousXmlElement.equals(XML_SEEING)) {
                ...
                _avTable.noNotifySet(ATTR_SEEING, "" + getSeeingFromMax(max),
                                     0);
