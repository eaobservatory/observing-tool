...
public class SpIterPOL extends SpIterConfigObs {
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "instPOL", "POL");
    ...
    public IterConfigItem[] getAvailableItems() {
        ...
        IterConfigItem iciWPLAngle = new IterConfigItem("Waveplate Angle",
                "POLIter", ALLOWED_ANGLES);
    ...
    public void setContinuousSpin(boolean enabled) {
        if (enabled) {
            ...
            getTable().set(SpIterPOL.CONTINUOUS_SPIN, 0.0);
        } else {
