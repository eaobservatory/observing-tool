...
public class AcsisTranslator extends SpInstHeterodyne
{
	...
	public void translate( SpObs spObs , SpInstHeterodyne instHeterodyne , SpIterObserveBase iterComp , String dir ) throws IOException , IllegalStateException
	{
		...
		//
		// This will become obsolete and can be removed if users specify a path for every XML entity
		// that is included in the XML generated by this translator.
		String baseDir = System.getProperty( "ot.cfgdir" ) ;
		...
		try
		{
			while( ( block = instCfg.readBlock() ) != null )
			{
				...
				else if( instInfo.getKeyword().equalsIgnoreCase( "DR_RECIPE_FILES" ) )
				{
					...
					// cfg file have the same values as OBS_MODE_ constants in this class.
	...
	public String get_line_list()
	{
		...
		for( int i = 0 ; i < getNumSubSystems() ; i++ )
		{
			...
			xmlBuffer.append( indent + "<rest_frequency id=\"" + restFreqRefs[ i ] + "\" units=\"GHz\">" + ( getRestFrequency( i ) / 1.0E9 ) + "</rest_frequency>\n" ) ;
		}
	...
	public String getACSIS_corr()
	{
		...
		// One of the subband centres will have to be duplicated in order to get 4 lo2 values.
		...
		for( int i = 0 ; i < 32 ; i++ )
		{
			bw_MHz = getIndividualSubBandWidth( i % numSubSystems ) / 1.E6 ;

			if( bw_MHz >= 1000. )
				bw_mode = "" + ( ( int )( bw_MHz / 1000. ) ) + "GHz" ;
			else
				bw_mode = "" + ( ( int )bw_MHz ) + "MHz" ;

			bw_mode += "x" + getIndividualSubBandChannels( i % numSubSystems ) ;

			xmlBuffer.append( indent + "  <cm id=\"" + ( i + 1 ) + "\" bw_mode=\"" + bw_mode + "\"/>\n" ) ;
		}
	...
	public String getACSIS_IF()
	{
		...
		for( int i = 0 ; i < 32 ; i++ )
		{
			...
			xmlBuffer.append( indent + "  <dcm id=\"" + ( i + 1 ) + "\" bw_mode=\"" + bw_mode + "\"/>\n" ) ;
		}
	 ...
	 *
	 * A subsystem with <i>n</i> hybrid subbands (<i>n &gt ; 1</i>) results in <i>n + 1</i>
	 * &lt ;spectral_window&gt ; elements in the XML. One representing the subsystem as a
	 ...
	 * only one &lt ;spectral_window&gt ; element.
	 *
	 * @see spectralWindowToXML(java.lang.String,int,java.lang.String,java.lang.String,int)
	 */
	public String subSystemToXML( String restFrequencyId , int sideband , String indent , int subsystem )
	{
		StringBuffer xmlBuffer = new StringBuffer() ;
	 ...
	 *
	 * In hybrid mode each individual subband gets its own &lt ;spectral_window&gt ; element
	 * Additionally the combined subsystem gets a &lt ;spectral_window&gt ; element as well.
	...
	public String spectralWindowToXML( String restFrequencyId , int sideband , String subSystemXML , String indent , int subsystem , int hybridSubBand )
	{
		...
		// Used for memory allocation. Number of channels is not reduced according to overlap to be on the save side.
		...
		// Note that hybridSubBand should always be SUBSYSTEM_INDEX if getNumHybridSubBands() is 1
		...
		else
		{
			...
			referencePixel = ( int )( ( ( ( getBandWidth( subsystem ) / getNumHybridSubBands( subsystem ) ) * ( ( getNumHybridSubBands( subsystem ) / 2.0 ) - hybridSubBand ) ) + ( getOverlap( subsystem ) / 2.0 ) ) / channelWidth ) ;
		}
		...
		// But this is not properly by the software that handles the spectral_window XML. So it is left empty for now.
	...
	public String getConfigLabel()
	{
		...
		{
			return getConfigLabel( getFrontEnd() , getNumSubSystems() , getNumHybridSubBands( 0 ) , getBandWidth( 0 ) ) ;
		}
	}
	...
	public static String getObserveMode( SpIterObserveBase iterComp )
	{
		...
		else if( iterComp instanceof SpIterStareObs )
	...
	/**
	 * Returns &lt ;BASE&gt ; elements in the SpTelescopeObsComp XML.
	 */
	public static String getTcsXml( SpTelescopeObsComp telescopeObsComp )
	{
		...
		while( stringTokenizer.hasMoreTokens() )
		{
			...
			if( ( !line.trim().equals( "" ) ) && ( !line.trim().startsWith( "<?xml" ) ) && ( !line.trim().startsWith( "<SpTelescopeObsComp" ) ) && ( !line.trim().startsWith( "</SpTelescopeObsComp" ) ) && ( !line.trim().startsWith( "<meta" ) ) )
	...
	/**
	 * Returns &lt ;SECONDARY&gt ; element in the SpTelescopeObsComp XML.
	 *
	 * The &lt ;SECONDARY&gt ; element is defined in
	 * <a href="http://www.jach.hawaii.edu/JACdocs/JCMT/OCS/ICD/006">
	 * JCMT TCS/OCS Interfaces</a>.
	 */
	public static String getTcsXml( SpIterChop iterChop )
	{
		...
		while( stringTokenizer.hasMoreTokens() )
		{
			...
			// There might be <SECONDARY> in the XML of the child nodes which is included
			// in iterComp.toXML() but that will be dealt with separately later.
	 ...
	 * <a href="http://www.jach.hawaii.edu/JACdocs/JCMT/OCS/ICD/006">
	 * JCMT TCS/OCS Interfaces</a> compatible &lt ;obsArea&gt ; XML element.
	 *
	...
	public static String getTcsXml( SpIterComp iterComp , boolean ignorePA )
	{
		...
		// Ignore the <obsArea> and </obsArea> lines and record only what is inside the <obsArea> element
		while( stringTokenizer.hasMoreTokens() )
		{
			line = stringTokenizer.nextToken() ;

			// Return at the end of the first <obsArea> There might be <obsArea> in the XML
			// of the child nodes which is included in iterComp.toXML() but that will be dealt with separately later.
	...
	/**
	 * Returns &lt ;JIGGLE&gt ; and &lt ;TIMING&gt ; element in the SpTelescopeObsComp XML.
	 *
	 * The &lt ;JIGGLE&gt ; and &lt ;TIMING&gt ; element is defined in
	 * <a href="http://www.jach.hawaii.edu/JACdocs/JCMT/OCS/ICD/006">
	 * JCMT TCS/OCS Interfaces</a>.
	 *
	...
	public static String[] getTcsXml( SpIterJiggleObs iterComp )
	{
		...
		result[ 0 ] = "    <JIGGLE NAME=\"" + iterComp.getJigglePattern() + "\" SYSTEM=\"" + iterComp.getCoordSys() + "\" SCALE=\"1.0\">\n" + "      <PA>" + iterComp.getPosAngle() + "</PA>\n" + "    </JIGGLE>" ;
