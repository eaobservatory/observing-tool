...
final public class JHLauncher implements java.io.Serializable
{
	...
	private class OpenPageListener implements ActionListener
	{
		...
		public void actionPerformed( ActionEvent e )
		{
			if( opDialog == null )
			{
				opDialog = new JDialog( JHLauncher.this.getJFrame() , "Open Page" , false );
	...
	private class SetFontListener implements ActionListener , TreeSelectionListener , ItemListener
	{
		...
		public void actionPerformed( ActionEvent e )
		{
			...
			{
				JOptionPane.showMessageDialog( null , "Setting Fonts on JDK1.1 not allowed" , "Set Font..." , JOptionPane.ERROR_MESSAGE );
				return;
			}
			if( sfDialog == null )
			{
				sfDialog = new JDialog( JHLauncher.this.getJFrame() , "Set Font..." , false );
		...
		private void initSetFontComponents()
		{
			...
			{
				DefaultMutableTreeNode node = new DefaultMutableTreeNode( new MyFont( fonts[ i ] ) );
	...
	private JMenuBar createMenuBar()
	{
		...
		mi.addActionListener( new ActionListener()
		{
			public void actionPerformed( ActionEvent e )
			{
				// setVisible(false) is used instead of dispose() because dispose() causes the OT to get stuck (almost freeze).
				// In order to prevent having too many invisible help frames the same help frame 
				// (actually the same JHLauncher) is being reused, i.e. setVisible(true), 
				// whenever the user selects the help menu again.
	...
	private void initializeSDGUI()
	{
		...
		if( !on12 )
		{
			JOptionPane.showMessageDialog( null , "Cannot set the HelpSet on JDK1.1 Platforms" , "Error" , JOptionPane.ERROR_MESSAGE );
			if( jh == null )
				frame.setVisible( false ); //System.exit(0);
			else
				return;
		...
		panel.setBorder( new TitledBorder( BorderFactory.createEtchedBorder() , "HelpSet Information" ) );
	...
	private class DisplayAction implements ActionListener
	{
		public void actionPerformed( ActionEvent e )
		{
			...
			if( url == null )
			{
				JOptionPane.showMessageDialog( selectionDialog , "HelpSet not found" , "Error" , JOptionPane.ERROR_MESSAGE );
				return;
			}
