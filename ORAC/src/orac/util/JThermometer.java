...
public class JThermometer extends JComponent {
    ...
    private void paintThermometer(Graphics2D g2) {
        ...
        RoundRectangle2D.Double outer = 
            new RoundRectangle2D.Double( firstX, firstY,
                    lastX/*-insets.right*/, _defaultHeight,
                    _defaultHeight, _defaultHeight);

        // Work out the inner thermometer
        int endMercury = (int) ((double)_brm.getExtent() /
                _brm.getMaximum() * (lastX /*- insets.right*/) );
        RoundRectangle2D.Double inner =
            new RoundRectangle2D.Double(firstX+1, firstY+1,
                    endMercury, _defaultHeight-2,
                    _defaultHeight-2, _defaultHeight-2);
...
//         int trans = firstX + _defaultHeight + 10;
...
//         g2.setFont(new Font("dialog", Font.PLAIN, 10));
        ...
        for ( int i=0; i<=_nTicks; i++ ) {
            ...
            double pos = i*((double)lastX /*- insets.left*/) / _nTicks;
