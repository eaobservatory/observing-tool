...
public class CGS4Validation implements InstrumentValidation {
    ...
    public void checkInstrument(SpInstObsComp instObsComp, Vector report) {
      ...
      double posAngle          = spInstCGS4.getPosAngleDegrees();
      ...
      if((grating.equals("40lpmm") || grating.equals("150lpmm")) && (order > ORDER_FOR_40_OR_150_GRATING_MAX)) {
        report.add(new ErrorMessage(ErrorMessage.ERROR,
	                            "CGS4", "order (with " + grating + " grating)",
	                            "<= " + ORDER_FOR_40_OR_150_GRATING_MAX,
		                    "" + order));
      }
      else if((grating.equals("echelle")) && (order < ORDER_FOR_ECHELLE_GRATING_MIN)) {
        report.add(new ErrorMessage(ErrorMessage.ERROR,
	                            "CGS4", "order (with " + grating + " grating)",
	                            ">= " + ORDER_FOR_ECHELLE_GRATING_MIN,
				    "" + order));
      }
      ...
      if(expTime < EXPOSURE_TIME_MIN) {
        ...
        report.add(new ErrorMessage(ErrorMessage.WARNING,
	                            "CGS4",
	                            "found exposure time " + expTime + ", subarray needs to be set by TSS."));
      }

      // Checking for invalid ND filter/grating combination.
      if((ndFilter == true) && grating.equals("echelle")) {
        report.add(new ErrorMessage(ErrorMessage.ERROR,
	                            "CGS4",
	                            "ND filter cannot be selected in combination with grating \"echelle\""));
      }
