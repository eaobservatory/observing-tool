...
public class SpIterCGS4CalObs extends SpIterObserveBase
{
   ...
   public static final SpType SP_TYPE =
        SpType.create(SpType.ITERATOR_COMPONENT_TYPE, "CGS4calUnitObs", "CGS4 Cal Unit Observe");
...
getExposureTime()
{
  double et = _avTable.getDouble(SpCGS4CalUnitConstants.ATTR_EXPOSURE_TIME, 0.0);
  if (et == 0.0) {
...
getCvfWavelength()
{
   double cwl = _avTable.getDouble(SpCGS4CalUnitConstants.ATTR_CVF_WAVELENGTH, 0.0);
   if (cwl < 0.5) {
...
getDefaultMode()
{
   ...
   if (inst != null) {
      ...
      for (int i =0; i<MODES.length; i++) {
         if (instmode.equals(MODES[i])) mode = instmode;
      }
   }
   return mode;
}
