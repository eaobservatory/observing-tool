...
_thisFirstElement()
{
   ...
   _values[11] = new SpIterValue(SpMichelleCalConstants.ATTR_NULL_EXPOSURES,
   ...
   _values[14] = new SpIterValue(SpMichelleCalConstants.ATTR_CHOP_FREQUENCY,
      ico.W_chopFrequency);
...
getExposureTime()
{
  double et = _avTable.getDouble(SpMichelleCalConstants.ATTR_EXPOSURE_TIME, 0.0);
  if (et == 0.0) {
...
getFlatSourceChoices()
{
  ...
  for (int i = 0;i<ni;i++) choices[i] = inst.getFlatList()[i];
  return choices;
}
