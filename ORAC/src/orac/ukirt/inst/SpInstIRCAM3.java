...
public final class SpInstIRCAM3 extends SpUKIRTInstObsComp
{
	...
	public static double DEFBIASEXPTIME = 0. ;
	public static int DEFBIASCOADDS = 0 ;
	static String FILTER_NONE = "None" ;

	public static final SpType SP_TYPE = SpType.create( SpType.OBSERVATION_COMPONENT_TYPE , "inst.IRCAM3" , "IRCAM3" ) ;
	...
	/**
	 * Set the exposure time. Overrides the one in SpUKIRTInstObsComp.
	 */
	 ...
	 * and waveband.  It overrides the one in SpUKIRTInstObsComp.
	 */
	public double getDefaultExpTime()
	{
		...
		{
			System.out.println( "Array index 0 out of bounds ; there is probably " + "something seriously wrong with the ircam3.cfg file." ) ;
		}
	...
	public int getDefaultCoadds()
	{
		...
		// Check filter for which change et*coadd total - and et for which  if > 4, then just use 1.
	...
	/**
	 * Set the readout area.  This also updates the instrument-aperture
	 * X,Y values.
	 */
	...
	public String getDefaultAcqMode()
	{
		...
		try
		{
			...
			mode = ( String )delut.elementAt( row , 1 ) ;
		}
		catch( ArrayIndexOutOfBoundsException ex )
		{
			System.out.println( "Array index 0 out of bounds ; there is probably " + "something seriously wrong with the ircam3.cfg file" ) ;
	...
	/**
	 * Set the instrument apertures.  It is called whenever something changes that 
	 * causes a change in instrument aperture.   X, Y depend on readout area,
	 * filter, and polariser.  Positional angle is not included as it's
