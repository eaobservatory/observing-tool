 ...
 * areas.
 */
final class TpeSurveyContainerAreaDragObject
{
	int _xb , _yb ;
	double _angle = 0 ;
	 ...
	 */
	public TpeSurveyContainerAreaDragObject( int xb , int yb , int x , int y )
	{
		_xb = xb ;
		_yb = yb ;
		_angle = getAngle( x , y ) ;
 ...
 */
public class TpeSurveyContainerAreaFeature extends TpeImageFeature implements TpeDraggableFeature
{
	private TpeSurveyContainerArea _scanArea ;
	private PolygonD _scanAreaPD ;
	private PolygonD _tickMarkPD ;
	private SpSurveyContainer _surveyContainer ;
	...
	private TpeSurveyContainerAreaDragObject _dragObject ;
	 ...
	 */
	public TpeSurveyContainerAreaFeature()
	{
		super( "Survey Container Area" , "Survey Container area." ) ;
	}
	...
	private boolean _calc( FitsImageInfo fii )
	{
		...
		SpSurveyContainer iterRaster = null ;
		SpItem item = _iw.getBaseItem() ;
		
		Vector containers = SpTreeMan.findAllItems( item , SpSurveyContainer.class.getName() ) ;
		
		if( containers.size() == 1 )
			iterRaster = ( SpSurveyContainer )containers.elementAt( 0 ) ;
		...
		if( _scanArea == null )
		{
			_scanArea = new TpeSurveyContainerArea() ;
			if( _scanArea == null )
				return false ;
		}
		...
		if( _valid && !updated && ( _scanAreaPD != null ) && ( iterRaster == _surveyContainer ) )
			return true ;

		_surveyContainer = iterRaster ;
	...
	public boolean dragStart( FitsMouseEvent fme , FitsImageInfo fii )
	{
		...
		for( int i = 0 ; i < ( _scanAreaPD.npoints - 1 ) ; ++i )
		{
			...
			_dragObject = new TpeSurveyContainerAreaDragObject( ( int )p.x , ( int )p.y , cornerx , cornery ) ;
		...
		if( _dragObject == null )
		{
			...
			if( dx <= MARKER_SIZE + 2 )
			{
				...
				{
					Point2D.Double p = fii.baseScreenPos ;
					_dragObject = new TpeSurveyContainerAreaDragObject( ( int )p.x , ( int )p.y , x , y ) ;
				}
			}
		...
		if( _dragging )
		{
			_dragX = fme.xWidget ;
			_dragY = fme.yWidget ;
			_surveyContainer.getAvEditFSM().setEachEditNotifies( false ) ;
		}
		return _dragging ;
	}
	...
	public void dragStop( FitsMouseEvent fme )
	{
		if( _dragObject != null )
		{
			_surveyContainer.getAvEditFSM().setEachEditNotifies( true ) ;
			_dragging = false ;
			drag( fme ) ;
			_dragObject = null ;
		}
