...
public class TpeJcmtChopFeature extends TpeChopFeature
{
	...
	public void draw( Graphics g , FitsImageInfo fii )
	{
		...
		if( !containsRasterObservation( _iterChop ) )
		{
			...
			double chop2X = _iterChop.getThrow( chopStepIndex ) * Math.sin( ( _iterChop.getAngle( chopStepIndex ) / 360 ) * ( Math.PI * 2.0 ) ) ;
			double chop2Y = _iterChop.getThrow( chopStepIndex ) * Math.cos( ( _iterChop.getAngle( chopStepIndex ) / 360 ) * ( Math.PI * 2.0 ) ) ;
			...
			// There is an instrument in scope then draw the science area around the base position and chop positions.
			if( ( _iw.getInstrumentItem() != null ) && ( ( scienceArea = _iw.getInstrumentItem().getScienceArea() ) != null ) )
			{

				// ScienceArea is circular (approximately circular detector/array as opposed to circular representation of science area due to Ax/El mounting)
				if( scienceArea.length == 1 )
				{
					...
					g.drawArc( ( int )( chop2X - radius ) , ( int )( chop2Y - radius ) , ( int )( 2 * radius ) , ( int )( 2 * radius ) , 0 , 360 ) ;
				}
				...
				if( scienceArea.length == 2 )
				{
					...
					else
					{
						...
						g.drawRect( ( int )( chop2X - ( w / 2. ) ) , ( int )( chop2Y - ( h / 2. ) ) , ( int )w , ( int )h ) ;
					}
				}
			}
		...
		else
		{
			...
			double chop1X = _iterChop.getThrow( chopStepIndex ) * Math.sin( ( ( 180 + _iterChop.getAngle( chopStepIndex ) ) / 360 ) * ( Math.PI * 2. ) ) ;
			double chop1Y = _iterChop.getThrow( chopStepIndex ) * Math.cos( ( ( 180 + _iterChop.getAngle( chopStepIndex ) ) / 360 ) * ( Math.PI * 2. ) ) ;
			...
			double chop2X = _iterChop.getThrow( chopStepIndex ) * Math.sin( ( ( 180 + _iterChop.getAngle( chopStepIndex ) ) / 360 ) * ( Math.PI * 2. ) ) ;
			double chop2Y = _iterChop.getThrow( chopStepIndex ) * Math.cos( ( ( 180 + _iterChop.getAngle( chopStepIndex ) ) / 360 ) * ( Math.PI * 2. ) ) ;
			...
			if( ( _iw.getInstrumentItem() != null ) && ( ( scienceArea = _iw.getInstrumentItem().getScienceArea() ) != null ) )
			{
				// ScienceArea is circular (approximately circular detector/array as opposed to circular representation of science area due to Az/El mounting)
				if( scienceArea.length == 1 )
				{
					double radius = scienceArea[ 0 ] * fii.pixelsPerArcsec ;

					g.drawArc( ( int )( chop1X - radius ) , ( int )( chop1Y - radius ) , ( int )( 2 * radius ) , ( int )( 2 * radius ) , 0 , 360 ) ;
					g.drawArc( ( int )( chop2X - radius ) , ( int )( chop2Y - radius ) , ( int )( 2 * radius ) , ( int )( 2 * radius ) , 0 , 360 ) ;
				}
				...
				if( scienceArea.length == 2 )
				{
					...
					else
					{
						...
						g.drawRect( ( int )( chop1X - ( w / 2.0 ) ) , ( int )( chop1Y - ( h / 2. ) ) , ( int )w , ( int )h ) ;
						g.drawRect( ( int )( chop2X - ( w / 2.0 ) ) , ( int )( chop2Y - ( h / 2. ) ) , ( int )w , ( int )h ) ;
					}
				}
			}
			...
			if( _drawAsCircle )
			{
				...
				_chopInnerRadius = ( _iterChop.getThrow( chopStepIndex ) / 2. ) - scienceAreaRadius ;
				_chopInnerRadius *= fii.pixelsPerArcsec ;

				_chopOuterRadius = ( _iterChop.getThrow( chopStepIndex ) / 2. ) + scienceAreaRadius ;
