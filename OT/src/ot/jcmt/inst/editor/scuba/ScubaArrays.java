...
public class ScubaArrays extends JPanel implements ActionListener, MouseListener {
  ...
  private BolometerArray _bolometerSHORT =
                            new BolometerArray(Bolometer.BOLOMETER_SHORT,
			                       new int[] { 201, 365, 365, 207,  55,  55 },
                                               new int[] {  31, 124, 305, 393, 305, 124 },
					       6);

  private BolometerArray _bolometerLONG  =
                            new BolometerArray(Bolometer.BOLOMETER_LONG,
			                       new int[] { 713, 920, 920, 717, 520, 520 },
                                               new int[] {   0, 106, 345, 463, 345, 125 },
					       6);

  private Vector _bolometerListeners = new Vector();
    ...
    while(true) {
	  ...
	  data[3] = stringTokenizer.nextToken();  // Bolometer du4
          ...
          if(type != Bolometer.BOLOMETER_NONE) {
            if((type & Bolometer.BOLOMETER_SHORT) != 0) {
              xShift = 0.0;
            }
            else {
              xShift = 500.0;
            ...
            // Use P1100, P1300, P2000 (i.e. type stored in data[1]) for the bolometers
	    // outside the short wave and long wave arrays.
  ...
  public void enableBolometers(int bolometerTypes) {
    ...
    for(int i = 0; i < _bolometers.length; i++) {
      if((_bolometers[i].getBolometerType() & bolometerTypes) != 0) {
        _bolometers[i].setEnabled(true);
      }
      else {
        _bolometers[i].setEnabled(false);
      }
    }
  }
  ...
  public void update(int bolometerTypes, Vector selectedBolometers, String primaryBolometer) {
    ...
    if(((Bolometer.BOLOMETER_SHORT & bolometerTypes) != 0) ||
       ((Bolometer.BOLOMETER_LONG  & bolometerTypes) != 0)) {

      setArraySelectionEnabled(true);
    }
    else {
      setArraySelectionEnabled(false);
    }
  ...
  public static void main(String [] args) {
	...
	"\n  P2000 = 16");
      ...
      if(args.length < 2) {
        ...
        pixelNames.enableBolometers(Bolometer.BOLOMETER_SHORT +
	                            Bolometer.BOLOMETER_LONG);
      }
      else {
        pixelNames.setArraySelectionEnabled(false);
        pixelNames.enableBolometers(Bolometer.BOLOMETER_P1100 +
	                            Bolometer.BOLOMETER_P1350 +
				    Bolometer.BOLOMETER_P2000);
      }
