...
public final class EdCompInstIRCAM3 extends EdCompInstBase
	...
	ddlbw.setChoices(SpInstIRCAM3.READAREAS.getColumn(0));

	ddlbw = _w.sourceMagnitude;
	ddlbw.setChoices(SpInstIRCAM3.SRCMAGS);

	ddlbw = _w.acquisitionMode;
	ddlbw.setChoices(SpInstIRCAM3.MODES);

	ddlbw = _w.polariser;
	ddlbw.setChoices(SpInstIRCAM3.POLARISERS.getColumn(0));
	...
	String filter = ((SpInstIRCAM3) _spItem).getFilter();
	stw.setText(filter);
	...
	SpInstIRCAM3 instIRCAM3 = (SpInstIRCAM3) _spItem;

	DropDownListBoxWidgetExt ddlbw;

	ddlbw = _w.readoutArea;
	ddlbw.setValue( instIRCAM3.getReadoutArea() );

	ddlbw        = _w.acquisitionMode;
	ddlbw.setValue( instIRCAM3.getAcqMode() );

	ddlbw         = _w.sourceMagnitude;
	ddlbw.setValue( instIRCAM3.getSourceMagnitude() );
	...
	_edStareCapability._updateWidgets(this, instIRCAM3.getStareCapability());
    }
	...
	double[] scienceArea = instIRCAM3.getScienceArea();
	...
	double d = instIRCAM3.getExpTime();
	...
	tbw = _w.coadds; 
	int coadds = instIRCAM3.getNoCoadds();
	// _instIRCAM3.setNoCoadds(coadds);
	tbw.setText (Integer.toString(coadds));

      System.out.println("_updateExpWidgets has been called. exposure = " + instIRCAM3.getExpTime() + ", coadds = " + instIRCAM3.getNoCoadds());
    }
	...
	String curValue = instIRCAM3.getFilter();
	if ((curValue != null) && (curValue.equals(filter))) {
	    return;
	}

	instIRCAM3.setFilter(filter);
    ...
    public TextBoxWidgetExt getPosAngleTextBox() {
      ...
      //                                                  "But it is not implemented by EdCompInstIRCAM3.\n" +
    ...
    */
	...
	if (ddlbw == _w.sourceMagnitude) {
	    instIRCAM3.setSourceMagnitude(val);
	    _updateExpWidgets();
	    return;
	}
    }
