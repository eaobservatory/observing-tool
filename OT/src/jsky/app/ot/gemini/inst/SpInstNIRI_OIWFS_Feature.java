...
class SpInstNIRI_OIWFS_Obscured
{
   ...
   // range (circle).
   //
   private static PolygonD
   _calculateObscured(
       double centerSquareWidth, double crossWidth, double circleDiameter)
   {
      ...
      // Quadrant IV points
      xpoints[ 4] =  xpoints[3];  xpoints[ 5] =  xpoints[2];
      ypoints[ 4] = -ypoints[3];  ypoints[ 5] = -ypoints[2];
   
      xpoints[ 6] =  xpoints[1];  xpoints[ 7] =  xpoints[0];
      ypoints[ 6] = -ypoints[1];  ypoints[ 7] = -ypoints[0];
   
   
      // Quadrant III points
      xpoints[ 8] = -xpoints[0];  xpoints[ 9] = -xpoints[1];
      ypoints[ 8] = -ypoints[0];  ypoints[ 9] = -ypoints[1];
   
      xpoints[10] = -xpoints[2];  xpoints[11] = -xpoints[3];
      ypoints[10] = -ypoints[2];  ypoints[11] = -ypoints[3];
   
   
      // Quadrant II points
      xpoints[12] = -xpoints[3];  xpoints[13] = -xpoints[2];
      ypoints[12] =  ypoints[3];  ypoints[13] =  ypoints[2];
   
      xpoints[14] = -xpoints[1];  xpoints[15] = -xpoints[0];
      ypoints[14] =  ypoints[1];  ypoints[15] =  ypoints[0];
      ...
      double sin = Angle.sinRadians( Math.PI / 4.0 );
      double cos = Angle.cosRadians( Math.PI / 4.0 );
   ...
   //
   private static PolygonD
   _calculateF6WithVignetting(double centerSquareWidth)
   {
      ...
      xpoints[0] =  halfSquareWidth;   xpoints[1] = -halfSquareWidth;
      ypoints[0] =  halfSquareWidth;   ypoints[1] =  halfSquareWidth;

      xpoints[2] = -halfSquareWidth;   xpoints[3] =  halfSquareWidth;
      ypoints[2] = -halfSquareWidth;   ypoints[3] = -halfSquareWidth;
   ...
   //
   // _ignoreVig[CAMERA_F6 ] = _calculateObscured(123.0, 2.0, 210.0);
   // _ignoreVig[CAMERA_F14] = _calculateObscured( 51.0, 2.0, 210.0);
   // _ignoreVig[CAMERA_F32] = _ignoreVig[CAMERA_F14];
   // _withVig[CAMERA_F6 ]   = _calculateF6WithVignetting(148.0);
   // _withVig[CAMERA_F14]   = _calculateObsured(61.0, 15.0, 168.0);
   // _withVig[CAMERA_F32]   = _withVig[CAMERA_F14];
   //
   private static PolygonD
   _getObscured(int camera, boolean withVignetting)
   {
      PolygonD pd = null;
      ...
      if (withVignetting) {
        ...
        if (pd == null) {
           switch (camera) {
              case SpInstNIRI.CAMERA_F6:
                 _withVig[SpInstNIRI.CAMERA_F6] =
                                            _calculateF6WithVignetting(148.0);
                 break;
              case SpInstNIRI.CAMERA_F14:
              case SpInstNIRI.CAMERA_F32:
                 _withVig[SpInstNIRI.CAMERA_F14] =
                                        _calculateObscured(61.0, 15.0, 168.0);
                 _withVig[SpInstNIRI.CAMERA_F32] =
                                              _withVig[SpInstNIRI.CAMERA_F14];
                 break;
           }
           pd = _withVig[camera];
        ...
        if (pd == null) {
           switch (camera) {
              case SpInstNIRI.CAMERA_F6:
                 _ignoreVig[SpInstNIRI.CAMERA_F6] =
                                        _calculateObscured(123.0, 2.0, 210.0);
                 break;
              case SpInstNIRI.CAMERA_F14:
              case SpInstNIRI.CAMERA_F32:
                 _ignoreVig[SpInstNIRI.CAMERA_F14] =
                                         _calculateObscured(51.0, 2.0, 210.0);
                 _ignoreVig[SpInstNIRI.CAMERA_F32] =
                                            _ignoreVig[SpInstNIRI.CAMERA_F14];
                 break;
           }
           pd = _ignoreVig[camera];
   ...
   getObscured(int camera, boolean withVignetting,
               double pixelsPerArcsec, Point2D.Double basePos, double angle)
   {
 ...
 */
public class SpInstNIRI_OIWFS_Feature extends TpeImageFeature
 ...
 */
public void
propertyChange(String propName)
{
   _iw.repaint(this);
}
 ...
 */
public void
setWithVignetting(boolean withVignetting)
{
 ...
 */
public void
setFillObscuredArea(boolean fill)
{
...
public void
validate(SpInstNIRI spNIRI, FitsImageInfo fii)
{
   ...
   if ((_obscuredPoly == null)                 ||
      ...
      _obscuredPoly = SpInstNIRI_OIWFS_Obscured.getObscured(
        cameraIndex, withVignetting, pixelsPerArcsec, baseScreenPos, angle);
   }
}
...
draw(Graphics g, FitsImageInfo fii)
{
   ...
   if (getWithVignetting()) {
      r = (int) (fii.pixelsPerArcsec *  84.0);
      d = (int) (fii.pixelsPerArcsec * 168.0);
   } else {
      r = (int) (fii.pixelsPerArcsec * 105.0);
      d = (int) (fii.pixelsPerArcsec * 210.0);
   }
