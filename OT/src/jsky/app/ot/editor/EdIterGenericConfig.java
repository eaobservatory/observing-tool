...
class ICTextBoxValueEditor extends ICValueEditor
    ...
    ICTextBoxValueEditor(EdIterGenericConfig ci,
			 JPanel gw, JLabel stw, TextBoxWidgetExt tbw) {
	super(ci, gw, stw);
	_textBox = tbw;
 ...
 */
public class EdIterGenericConfig extends OtItemEditor
	...
	//System.out.println(dbg + ".addConfigItem(" + ici + ")");
	...
	// the just added value.  That no longer works, so now just select nothing.
	//_initAvailableItems();
	// XXX allan: why deselect the selected item?
	//_unselectAvailableItems();
