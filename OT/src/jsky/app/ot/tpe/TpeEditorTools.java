...
final class TpeEditorTools {
    ...
    private static final String CREATE_BUTTON_ROOT = "createTool";
	...
	return new Enumeration() {  // XXX allan: should make 2 separate ToggleButtonPanels
	...
	if (_buttonMap.get(label) != null) return true;

	ToggleButtonWidget tbw = _allocateToolButton();
	if (tbw == null) return false;  // Never found a valid one
	...
	if (!(tif instanceof TpeCreateableFeature)) return;  // nothing to delete
	...
	if (!(tif instanceof TpeCreateableFeature)) return;  // nothing to disable
	...
	switch (_curMode) {
	case MODE_BROWSE:  _tpe.setCurrentMouseCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));   break;
	case MODE_DRAG:    _tpe.setCurrentMouseCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));      break;
	case MODE_ERASE:   _tpe.setCurrentMouseCursor(Cursor.getPredefinedCursor(Cursor.TEXT_CURSOR));      break;
	case MODE_CREATE:  _tpe.setCurrentMouseCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR)); break;
	}
    }
