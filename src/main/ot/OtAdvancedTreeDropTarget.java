 ...
 *
 * Implements a Drag and Drop features that was part of the FreeBongo OT but
 * are not part of the normal behaviour of the Swing Drag and Drop:
 * The user can choose whether to drop a source node after (i.e under) or
 * inside the target node.  The behaviour is indicated by a white arrow in
 * an orange blob. Pointing downwards for insertion after the target node
 * and pointing right/downwards ("South-East") for insertion inside
 * the target node.
 *
 * @author Martin Folger (M.Folger@roe.ac.uk)
 */
public class OtAdvancedTreeDropTarget extends OtTreeDropTarget implements
        KeyListener {
    ...
    /**
     * Determines the feedback the user gets during dragging a node over this
     * target node.
     *
     ...
     *
     * If the mouse is dragged to a position OFFSET_INSERT_INSIDE pixels or
     * more right of the left boundary of the target node then the
     * INSERT_INSIDE_GIF is displayed and the source node is inserted inside
     * the target node.  Otherwise the INSERT_AFTER_GIF is displayed and
     * the source node is inserted after the target node.
     */
    ...
    public OtAdvancedTreeDropTarget(OtTreeWidget treeWidget,
            OtAdvancedTreeDragSource source) {
        super(treeWidget);

        insertImage[INSERT_INSIDE] = (new ImageIcon(
                ObservingToolUtilities.resourceURL(INSERT_INSIDE_GIF)))
                .getImage();
        insertImage[INSERT_AFTER] = (new ImageIcon(
                ObservingToolUtilities.resourceURL(INSERT_AFTER_GIF)))
                .getImage();
        insertImage[INSERT_NO] = (new ImageIcon(
                ObservingToolUtilities.resourceURL(INSERT_NO_GIF))).getImage();
    ...
    public void dragOver(DropTargetDragEvent dtde) {
        ...
        try {
            location.x = tree.getPathBounds(tree.getPathForLocation(
                    dtde.getLocation().x, dtde.getLocation().y)).x;
            location.y = tree.getPathBounds(tree.getPathForLocation(
                    dtde.getLocation().x, dtde.getLocation().y)).y;
        } catch (NullPointerException e) {
            return;
        }

        // If mouse has moved over another node, repaint tree to get rid of
        // orange arrow that might still be on top of a previous node.
        ...
        dtde.getDropTargetContext().getComponent().getGraphics().drawImage(
                insertImage[currentInsertMode], location.x, location.y, null);
    ...
    protected boolean dropNodes(int action, Transferable transferable,
        ...
        OtDragDropObject ddo =
                (OtDragDropObject) transferable.getTransferData(
                        OtDragDropObject.dataFlavor);
        ...
        try {
            if (ownerTW == _spTree) {
                // The dragged item was owned by this tree, so move or copy
                // according to action.
                ...
                if (ownerTW != null) {
                    // Make a copy, since these items are owned by another
                    // tree.
                    for (int i = 0; i < newItems.length; ++i) {
                        newItems[i] = newItems[i].deepCopy();
                    }
             ...
             * resetProg seems to have caused the gaps in the tree between
             * obs components in a group.  Unless other problems araise
             * updateNodeExpansions should be used instead.  See also
             * revision 1.6 of orac3/OT/src/jsky/app/ot/OtTreeDropTarget.java
     ...
     */
    protected SpInsertData getSpInsertData(OtDragDropObject ddo,
            OtTreeNodeWidget node, int insertMode) {
    ...
    public void keyPressed(KeyEvent e) {
        if ((e.getKeyCode() == KeyEvent.VK_DELETE)
            ...
            try {
                ...
                // IllegalArgumentException is thrown if user attempts to
                // delete Science Program root node.
