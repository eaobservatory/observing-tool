...
/**
 * ReportBox has been rewritten to allow displaying different fonts and using
 * Java print routines.
 *
 * The code still contains some experimental bits which are mostly commented
 * out.
 *
...
public class ReportBox extends JFrame {
    ...
    public ReportBox(String message) {
        ...
        // Set frame bounds.
        // Choose frame height depending on the number of lines of the
        // document given frame width 480.   Multiplying the number of lines
        // by 16 and adding 100 gives roughly the right height.
        if (getNumberOfLines(_textPane.getText()) < 34) {
            setBounds(100, 100, 480,
                    100 + (getNumberOfLines(_textPane.getText()) * 16));
        } else {
    ...
    public void print() {
        ...
        PrintJob pj = getToolkit().getPrintJob(ReportBox.this,
                "OT Report (" + System.getProperty("user.name") + ")",
            ...
            if (fileName != null) {
                try {
                    if (description.equals(_richTextFormatDescription)) {
                        (new RTFEditorKit()).write(
                                new FileOutputStream(fileName),
                                _textPane.getDocument(), 0,
                                _textPane.getDocument().getLength());
