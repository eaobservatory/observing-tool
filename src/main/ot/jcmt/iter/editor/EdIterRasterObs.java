 ...
 */
public final class EdIterRasterObs extends EdIterJCMTGeneric implements
        Observer, OptionWidgetWatcher, KeyListener, CommandButtonWidgetWatcher,
        SpJCMTConstants {
    ...
    private final String[] SCAN_PA_CHOICES = {AUTOMATIC, ALONG_HEIGHT,
            ALONG_WIDTH, USER_DEF};
    ...
    private final static String[] HARP_RASTER_NAMES = {
            "1 array",
            "1/2 array",
            "1/4 array",
            "1/8 array",
    ...
    private static final double[] HARP_RASTER_STEPS = {
            1.0,
            0.5,
            0.25,
            0.125,
    ...
    static {
        for (int index = 0; index < HARP_RASTER_STEPS.length; index++) {
            double tempValue = SpIterRasterObs.HARP_FULL_ARRAY
                    * HARP_RASTER_STEPS[index];
    ...
    private void setDefaults() {
        ...
        for (int index = 0; index < HARP_RASTER_NAMES.length; index++) {
            _w.harpRasters.addChoice("step " + HARP_RASTER_NAMES[index]
                    + " (" + CoordConvert.round(HARP_RASTER_VALUES[index], 1)
                    + "\")");
        }
    }
    ...
    private void scuba2Setup() {
        ...
        boolean allowScan =
                SCAN_PATTERN_BOUS.equals(_iterObs.getScanStrategy());
    ...
    private void updateScanPanel() {
        if ((_iterObs.getScanAngles() == null)
                || (_iterObs.getScanAngles().size() == 0)) {
            _w.scanAngle.setEditable(false);
            _w.scanAngle.setValue(AUTOMATIC);

        } else if (_iterObs.getScanAngle(0)
                == (_iterObs.getPosAngle() + 90.0) % 180.0) {
            _w.scanAngle.setEditable(false);
    ...
    private void updateSizeOfPixels() {
        ...
        if (!scuba2) {
            ...
            BigDecimal bigSizeOfXPixel = width.divide(deeEx,
                    SpIterRasterObs.context);
            ...
            BigDecimal bigSizeOfYPixel = height.divide(deeWy,
                    SpIterRasterObs.context);
            ...
            if (!harp && sizeOfXPixel - Math.floor(sizeOfXPixel) != 0.0) {
            ...
            if (!harp && sizeOfYPixel - Math.floor(sizeOfYPixel) != 0.0) {
    ...
    public void textBoxKeyPress(TextBoxWidgetExt tbwe) {
        ...
        if (tbwe == _w.dx) {
            ...
            if (value.equals(ALONG_WIDTH)) {
                double pa = _iterObs.getPosAngle() + 90.0;
    ...
    public void dropDownListBoxAction(DropDownListBoxWidgetExt ddlbwe,
        ...
        if (ddlbwe == _w.scanSystem) {
            ...
            double value =
                    HARP_RASTER_VALUES[_w.harpRasters.getSelectedIndex()];
            ...
            if (SCAN_PATTERN_POINT.equals(value)) {
                ...
                if (integrations == null || integrations.trim().equals("")
                        || Integer.parseInt(integrations) < 1) {
    ...
    /**
     * This updates the time fields on heterodyne setups.
     *
     * It can only be used for heterodyne and just updates the non-editable
     * widgets. For ease of display, the fractional part is truncated to
     * 2 decimal places.
     */
