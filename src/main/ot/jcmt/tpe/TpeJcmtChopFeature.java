...
public class TpeJcmtChopFeature extends TpeChopFeature {
    ...
    public void draw(Graphics g, FitsImageInfo fii) {
        ...
        try {
            if (_iw.getInstrumentItem() != null) {
                scienceArea = ScienceArea.fromArray(_iw.getInstrumentItem()
                        .getScienceArea());
            }
        ...
        // as a small boxes.  The box at position (chop1X, chop1Y),
        ...
        // If the exact position is not known due to the Az/El coordinate
        // system then draw two circles around the base. The science area
        // is between these two circles.
        if (_drawAsCircle) {
            drawCircles(g);
        }
    }
    ...
    public void drag(FitsMouseEvent fme) {
        if (_dragObject != null) {
            ...
            if (containsRasterObservation(_iterChop)) {
                _iterChop.setThrow(getThrow(fme.xOffset, fme.yOffset) * 2.0,
                        _iterChop.getSelectedIndex());
            } else {
    ...
    protected boolean drawAsCircle(String coordFrame) {
        if (coordFrame.equals(SpJCMTConstants.CHOP_SYSTEMS[
                SpJCMTConstants.CHOP_SYSTEM_TARCKING])) {
            return false;
