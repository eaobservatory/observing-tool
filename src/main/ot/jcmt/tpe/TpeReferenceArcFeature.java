...
public class TpeReferenceArcFeature extends TpeImageFeature {
    ...
    public TpeReferenceArcFeature() {
        super("Ref. Arcs",
                "Arcs showing the possible positions of the reference beam.");
    ...
    private static class ArcRange {
        ...
        public static ArcRange merge(ArcRange a, ArcRange b) {
            a.checkOrder();
            b.checkOrder();
            return new ArcRange(Math.min(a.angleStart, b.angleStart), Math.max(
    ...
    private boolean calculateArcs() {
        ...
        if (!(telescope instanceof SpTelescopeObsComp)) {
            return false;
        }

        SpTelescopePos basePosition = ((SpTelescopeObsComp) telescope)
        ...
        if (basePosition.getCoordSys() == CoordSys.FK5) {
            ...
            try {
                RADec raDec = CoordConvert.convert(basePosition.getXaxis(),
                        basePosition.getYaxis(), basePosition.getCoordSys(),
                        CoordSys.FK5);
                declination = raDec.dec;

            } catch (UnsupportedOperationException e) {
                System.err.println(
                        "Wrong coordinate system in TpeReferenceArcFeature");
        ...
        if (sched != null) {
            ...
            if (meridianApproach != null
                    && SpSchedConstObsComp.SOURCE_RISING
                            .equals(meridianApproach)) {
                setting = false;
            } else if (meridianApproach != null
            ...
            if (minElevation != null) {
                try {
                    ...
                    System.err.println(
                            "Bad number format from"
                            + " SpSchedConstObsComp.minElevation()");
                }
            }
            ...
            if (maxElevation != null) {
                try {
                    ...
                    System.err.println("Bad number format from"
                            + " SpSchedConstObsComp.maxElevation()");
                }
        ...
        if (base instanceof SpIterFTS2Obs) {
            ...
            if (rising) {
                rangeRising = nasmythAngleRange(latitude, elevationMin,
                        elevationMax, declination, false, platform);
                rangeRising.offset(offsetAngle);
            }

            if (setting) {
                rangeSetting = nasmythAngleRange(latitude, elevationMin,
                        elevationMax, declination, true, platform);
                rangeSetting.offset(offsetAngle);
            }

            if (seeTransit && rising && setting) {
                arcs.add(ArcRange.merge(rangeRising, rangeSetting));
            } else {
                if (rising) {
    ...
    public void draw(Graphics g, FitsImageInfo fii) {
        ...
        // TODO: check that baseScreenPos always gives the position of
        // the actual target
     ...
     */
    private void drawTargetArea(Graphics g, double x0, double y0, double scale,
            double radFov) {

        g.drawOval((int) (x0 - scale * radFov), (int) (y0 - scale * radFov),
                (int) (2 * radFov * scale), (int) (2 * radFov * scale));
    }
     ...
     * @param scale pixels per arcsecond
     * @param radFov the radius of the field of view in arcseconds
     * @param radOffset the distance between the target and reference
     ...
     */
    private void drawReferenceArc(Graphics g, double x0, double y0,
            double scale, double radFov, double radOffset, ArcRange range) {
        ...
        double x1 = x0 + scale * radOffset
                * Math.sin(Math.PI * range.angleStart / 180);
        double y1 = y0 - scale * radOffset
                * Math.cos(Math.PI * range.angleStart / 180);
        ...
        g.drawArc((int) (x1 - (scale * radFov)),
                ...
                (int) (2 * scale * radFov),
                (int) (2 * scale * radFov),
                (int) (90 - range.angleStart), endCap);

        double x2 = x0 + scale * radOffset
                * Math.sin(Math.PI * range.angleEnd / 180);
        double y2 = y0 - scale * radOffset
                * Math.cos(Math.PI * range.angleEnd / 180);

        g.drawArc((int) (x2 - (scale * radFov)),
                (int) (y2 - (scale * radFov)),
                (int) (2 * scale * radFov),
                (int) (2 * scale * radFov),
                (int) (90 - range.angleEnd), -endCap);
    }
    ...
    private static double parallacticAngle(double latitudeDeg,
            double elevationDeg, double declinationDeg, boolean setting) {
        ...
        double verticalAngle = Math.acos((Math.sin(latitude)
     ...
     */
    private static double nasmythAngle(double latitudeDeg, double elevationDeg,
            double declinationDeg, boolean setting, NasmythPlatform platform) {

        double verticalAngle = parallacticAngle(latitudeDeg, elevationDeg,
                declinationDeg, setting);
    ...
    private static ArcRange nasmythAngleRange(double latitudeDeg,
        ...
        for (double elevationDeg = elevationDegMin;
                elevationDeg <= elevationDegMax; elevationDeg += 0.5) {

            double angle = nasmythAngle(latitudeDeg, elevationDeg,
                    declinationDeg, setting, platform);

            if (first || angle < angleMin) {
                angleMin = angle;
            }

            if (first || angle > angleMax) {
                angleMax = angle;
            }
    ...
    public static void main(String[] args) {
        ...
        for (double el = 0; el < maxEl; el++) {
            System.out.println(el + " "
                    + parallacticAngle(lat, el, dec, false) + " "
                    + nasmythAngle(lat, el, dec, false, NasmythPlatform.LEFT));
        }

        for (double el = maxEl; el > 0; el--) {
            System.out.println(el + " "
                    + parallacticAngle(lat, el, dec, true) + " "
                    + nasmythAngle(lat, el, dec, true, NasmythPlatform.LEFT));
        }
    }
}
