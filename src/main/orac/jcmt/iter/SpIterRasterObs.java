 ...
 *
 * The Raster iterator (ACSIS) and the Scan iterator share a lot of
 * functionality and should in future be either made the same class or
 * share code by other means such as inheritance.
 *
...
public class SpIterRasterObs extends SpIterJCMTObs implements
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "rasterObs", "Scan");
    ...
    public double getScanVelocity() {
        ...
        if (inst != null && inst instanceof SpInstSCUBA2) {
            double scanVelocity =
                    ((SpJCMTInstObsComp) inst).getDefaultScanVelocity();
    ...
    public double getScanDx() throws UnsupportedOperationException {
        ...
        if (inst == null) {
            throw new UnsupportedOperationException(
                    "Could not find instrument in scope.\n"
    ...
    public boolean setScanDx(double dx) throws UnsupportedOperationException {
        ...
        if (inst == null) {
            ...
            valid = setScanVelocity(dx * 200.0);
     ...
     * Convenience method.
     *
     * @return the number of samples per row, which means the longest edge.
     *
     * @throws java.lang.UnsupportedOperationException No instrument in scope
     */
     ...
     * Convenience method.
     *
     * @return the number of samples per column, which means the shortest edge.
     *
     * @throws java.lang.UnsupportedOperationException No instrument in scope
     */
    ...
    protected double numberOfSamplesOnSide(boolean row)
        ...
        samplesPerRow = (Math.floor(samplesPerRow / getScanDx())) + 1.0;
        samplesPerColumn = (Math.floor(samplesPerColumn / getScanDy())) + 1.0;
    ...
    public boolean swap() {
        ...
        double normalisedAngle = Math.abs((normalise(getScanAngle(0)))
                - (normalise(getPosAngle())));

        if ((getScanAngles() == null) || (getScanAngles().size() == 0)) {
            swap = columnGreaterThanRow;
        } else if (normalisedAngle < 45.0 || normalisedAngle > 135.0) {
            swap = true;
        }
    ...
    public double getElapsedTime() {
        ...
        if (instrument instanceof SpInstHeterodyne) {
            ...
            double time = getSecsPerRow() * samplesPerColumn + 80.0;
            return time;
    ...
    protected void processAvAttribute(String avAttr, String indent,
        ...
        // ATTR_SCANAREA_HEIGHT is an AV attribute that occurs once in a
        // SpIterOffset's AV table When processAvAttribute is called with
        // ATTR_SCANAREA_HEIGHT as avAttr then append the entire TCS XML
        // representation of this item to the xmlBuffer.  For all other calls
        // to processAvAttribute ignore the AV attributes, except meta
        // attributes such as ".gui.collapsed" which are delegated to the
        // super class.
    ...
    public void processXmlElementContent(String name, String value) {
        ...
        // Ignore XML elements whose do not contain characters themselves but
        // only XML attributes or XML child elements.
        if (name.equals(TX_OBS_AREA) || name.equals(TX_SCAN_AREA)
                || name.equals(TX_AREA) || name.equals(TX_SCAN)) {
            ...
            if ((_xmlPaAncestor != null) && _xmlPaAncestor.equals(TX_SCAN)) {
                if (getScanAngles() == null) {
                    ...
                    setScanAngle(Format.toDouble(value),
                            getScanAngles().size());
                }
            } else {
                setPosAngle(value);
    ...
    public void setupForHeterodyne() {
        if (_avTable.get(ATTR_SWITCHING_MODE) == null
            ...
            _avTable.noNotifySet(ATTR_SWITCHING_MODE, SWITCHING_MODE_POSITION,
                    0);
        }
    ...
    public void setupForSCUBA2() {
        ...
        _avTable.noNotifySet(ATTR_SCANAREA_SCAN_VELOCITY,
                "" + ((SpJCMTInstObsComp) SpTreeMan.findInstrument(this))
                        .getDefaultScanVelocity(), 0);

        String strategy = _avTable.get(ATTR_SCAN_STRATEGY);
        if (strategy == null || strategy.equals("")) {
            strategy = SCAN_STRATEGIES_SCUBA2[0];
            _avTable.noNotifySet(ATTR_SCAN_STRATEGY, SCAN_STRATEGIES_SCUBA2[0],
                    0);
        }
        ...
        if (strategy.equals(SCAN_PATTERN_POINT)) {
            ...
            if (_avTable.get(ATTR_SCAN_INTEGRATIONS) == null
                    || _avTable.get(ATTR_SCAN_INTEGRATIONS).equals("")) {
