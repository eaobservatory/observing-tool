...
public class SpItemDOM {
    ...
    private SpItemDOM(SpItem spItem, DocumentImpl ownerDoc) throws Exception {
        String classNameTag =
                spItem.getClass().getName().substring(
                        spItem.getClass().getName().lastIndexOf(".") + 1);
        ...
        while (children.hasMoreElements()) {
            _element.appendChild(
                    (new SpItemDOM(children.nextElement(),
                            ownerDoc)).getElement());
        }

        // Create item data node
        ElementImpl itemData =
                (ElementImpl) ownerDoc.createElement(SP_ITEM_DATA_TAG);
        ...
        itemData.setAttribute(SP_ITEM_PACKAGE,
                spItem.getClass().getName().substring(0,
                        spItem.getClass().getName().lastIndexOf(".")));
    ...
    public String toXML(String indent) throws Exception {
        ...
        // Serialize DOM
        OutputFormat format = new OutputFormat(_element.getOwnerDocument());
    ...
    /**
     * @return DOM element representing &lt;SP_ITEM_DATA_TAG&gt; if there is
    ...
    public ElementImpl getItemDataElement(ElementImpl element) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            if ((nodeList.item(i) instanceof ElementImpl)
                    && nodeList.item(i).getNodeName().equals(
                            SP_ITEM_DATA_TAG)) {
    ...
    public static void fillAvTable(String nameSoFar, ElementImpl remainingTree,
            SpAvTable avTab) {
        // Ignore ItemData
        ...
        if (remainingTree.getAttributes() != null
                && remainingTree.getAttributes().getLength() > 0) {
        ...
        // Check whether there are siblings with the same node name (tag name)
        // and set user data to string "#" + number
        setNumbers(nodeList);
                ...
                if (nodeList.item(i).getNodeName().equals("value")) {
                    if ((nodeList.item(i).getFirstChild() != null)
                            && (nodeList.item(i).getFirstChild().getNodeValue()
                                    != null)) {
                        valueVector.add(nodeList.item(i).getFirstChild()
                                .getNodeValue());
    ...
    public SpRootItem getSpItem() {
        ...
        try {
            ...
            // return deepCopy of spRootItem instead of spRootItem to prevent
            // bug that cases GUIDE row in target list table not to be
            // displayed.
    ...
    /**
     * @return SpItem if the DOM element represent an SpItem (i.e. has a
     *         &lt;SP_ITEM_DATA_TAG&gt;) or null otherwise.
     */
    protected SpItem getSpItem(ElementImpl element) {
        ElementImpl itemDataElement = getItemDataElement(element);

        // If element has no item data child then it does not represent
        // an SpItem.
        ...
        // Deal with attributes in SpItem XML Element (the ones who are
        // represented by attribute names starting with ":" in the SpAvTable)
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            ...
            System.out.println("   input_file: contains key value pairs"
                    + " for initialising the test table");
            return;
        }
        ...
        try {
            ...
            System.out.println("Problems loading properties form file "
                    + args[0] + ": " + e);
        }
    ...
    /**
     * Checks whether there are siblings with the same node name (tag name)
     * and set user data to string "#" + number so they can be save under
     * different names in the av table.
     */
    public static void setNumbers(NodeList siblings) {
        ...
        Hashtable<String, Vector<Node>> elementTable =
                new Hashtable<String, Vector<Node>>();

        for (int i = 0; i < siblings.getLength(); i++) {
            if (elementTable.get(siblings.item(i).getNodeName()) == null) {
                elementTable.put(siblings.item(i).getNodeName(),
                        new Vector<Node>());
            }

            elementTable.get(
                    siblings.item(i).getNodeName()).add(siblings.item(i));
        }
            ...
            if (elementVector.size() > 1) {
                for (int i = 0; i < elementVector.size(); i++) {
                    if (elementVector.get(i) instanceof ElementImpl) {
                        ((ElementImpl) elementVector.get(i)).setUserData(
                                "#" + i);
                    }
                }
            }
