...
class SpIterCGS4CalObsEnumeration extends SpIterEnumeration {
    ...
    protected SpIterStep _thisFirstElement() {
        ...
        _values[0] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_LAMP,
                ico.getLamp());
        _values[1] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_FILTER,
                ico.getFilter());
        _values[2] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_MODE,
                ico.getMode());
        _values[3] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_EXPOSURE_TIME,
                String.valueOf(ico.getExposureTime()));

        _values[4] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_COADDS,
                String.valueOf(ico.getCoadds()));

        _values[5] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_CVF_WAVELENGTH,
                String.valueOf(ico.getCvfWavelength()));
        _values[6] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_FLAT_SAMPLING,
                String.valueOf(ico.getFlatSampling()));
        _values[7] = new SpIterValue(
                SpCGS4CalUnitConstants.ATTR_NEUTRAL_DENSITY,
                String.valueOf(ico.getNdFilter()));
 ...
 */
@SuppressWarnings("serial")
public class SpIterCGS4CalObs extends SpIterObserveBase
    ...
    public static final SpType SP_TYPE = SpType.create(
            SpType.ITERATOR_COMPONENT_TYPE, "CGS4calUnitObs",
            "CGS4 Cal Unit Observe");
    ...
    public SpIterCGS4CalObs() {
        ...
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_MODE,
                DEFAULT_MODE, 0);
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_EXPOSURE_TIME,
                null, 0);
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_COADDS,
                DEFAULT_COADDS, 0);
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_CVF_WAVELENGTH,
                "0.0", 0);
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_FLAT_SAMPLING,
                "AS_OBJECT", 0);
        _avTable.noNotifySet(SpCGS4CalUnitConstants.ATTR_NEUTRAL_DENSITY,
                null, 0);
    }
    ...
    public double getExposureTime() {
        double et = _avTable.getDouble(
                SpCGS4CalUnitConstants.ATTR_EXPOSURE_TIME, 0.0);
    ...
    /**
     * Get a default value for the lamp.
     *
     * Need instrument disperser & wavelength.
     */
    ...
    public double getCvfWavelength() {
        double cwl = _avTable.getDouble(
                SpCGS4CalUnitConstants.ATTR_CVF_WAVELENGTH, 0.0);
    ...
    /**
     * Get a default value for the ND filter.
     *
     * Need wavelength and disperser.
     */
        ...
        if (getCalType() == FLAT) {
            ...
            if (inst != null) {
                try {
                    ...
                    if (instdi == 0) {
                        int pos = FLATLAMPS1.rangeInColumn(instcwl, 0);
                        ndFilter = Boolean.valueOf(
                                FLATLAMPS1.elementAt(pos, 3));

                    } else if (instdi == 1) {
                        int pos = FLATLAMPS2.rangeInColumn(instcwl, 0);
                        ndFilter = Boolean.valueOf(
                                FLATLAMPS2.elementAt(pos, 3));

                    } else if (instdi == 2) {
                        int pos = FLATLAMPS3.rangeInColumn(instcwl, 0);
                        ndFilter = Boolean.valueOf(
                                FLATLAMPS3.elementAt(pos, 3));
                    }
                } catch (Exception ex) {
                    ndFilter = false;
                }
    ...
    public void translate(Vector<String> v)
        ...
        if (inst == null || !(inst instanceof SpInstCGS4)) {
            throw new SpTranslationNotSupportedException(
                    "No CGS4 instrument component in scope");
        }

        // Get the current config items and then update it depending on the
        // type
        ...
        switch (getCalType()) {
            case FLAT:
                ...
                configTable.put("flatNeutralDensity",
                        Boolean.toString(getNdFilter()));
                break;

            case ARC:
                configTable.put("arcLamp",
                        getLamp().split("\\s")[0].toLowerCase());
                configTable.put("arcCvfWavelength", "" + getCvfWavelength());
                break;

            default:
                throw new SpTranslationNotSupportedException(
                        "CGS4 Cal Obs is not defined as flat or arc");
        }
        ...
        try {
            ...
            throw new SpTranslationNotSupportedException(
                    "Error writing CGS Calibration observation: "
                    + ioe.getMessage());
        }
