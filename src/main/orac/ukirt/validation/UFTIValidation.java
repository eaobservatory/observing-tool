...
public class UFTIValidation implements InstrumentValidation {
     ...
     * Given readoutAreas[i] and acqModes[i] check whether the exposure time
     * is at least minExpTimes[i].
     *
     * @see acqModes
     * @see minExpTimes
     */
    ...
    public void checkInstrument(SpInstObsComp instObsComp,
        ...
        if (expTime < expLimits[0] || expTime > expLimits[1]) {
            String message = "Exposure time of (" + expTime
                    + ") out of limits for current combination "
                    + "of readout area (" + readoutArea
                    + ") and mode (" + acqMode
                    + ").  Valid limits are "
                    + "(" + expLimits[0]
                    + ", " + expLimits[1] + ").";
