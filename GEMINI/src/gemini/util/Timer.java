 ...
 * Modeled after the Marimba Bongo timer by Arthur van Hoff.
 *
 * @author	Shane Walker
 */
...
public void
addClient(TimerClient client)
{
   addClient(client, System.currentTimeMillis());
}

public void
addClient(TimerClient client, long time)
{
   //System.out.println("add: ");
   _timerClientList.insert( client, time );
   //_timerClientList.printQueue();
   _notify();
}

public void
removeClient(TimerClient client)
{
...
_waitForNextTick()
{
   ...
   while (delay > 0) {
      ...
      // Can be notify()ed when the last client is removed - in which case
      // the client list will be empty.
      if (_timerClientList.isEmpty()) {
         return;
      }
