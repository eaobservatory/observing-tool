...
{
   public SpInsertInfo
   evalInsert(SpItem newItem, SpItem target)
   {
      return null;
   }
...
{
public SpInsertInfo
evalInsert(SpItem newChild, SpItem parent)
{
...
{
public SpInsertInfo
evalInsert(SpItem newItem, SpItem sibling)
{
...
/**
 * This is a utility class used for insertion and extraction of SpItems
 * to form legal Science Programs and Plans.  The SpItem base class
 * contains the primitive insert and extract methods.  This utility
 * uses those methods but performs additional evaluation to make sure
 * that only legal programs are created.
 *
 * @see SpItem#insert
 * @see SpItem#extract
 */
...
/**
 * The constructor is made private so that instances of this class
 * cannot be created.  This is a utility class and should be used
...
findConflictingObsComp(SpItem parent, SpObsComp spObsComp)
{
 ...
 */
public static SpTelescopeObsComp
findTargetListInContext(SpItem spItem)
{
 ...
 * searches the given scope.  It does not navigate the tree hierarchy.
 */
public static SpInstObsComp
findInstrumentInContext(SpItem spItem)
{
...
/**
 * Find the target list component associated with the given scope
 * scope of the given item.
 *
 * @param spItem the SpItem defining the scope to search
 */
public static SpTelescopeObsComp
findTargetList(SpItem spItem)
{
...
/**
 * Find an instrument observation component associated with the
 * scope of the given item.
 *
 * @param spItem the SpItem defining the scope to search
 */
public static SpInstObsComp
findInstrument(SpItem spItem)
{
 ...
 */
public static SpObsContextItem
findObsContext(SpItem spItem)
{
...
/**
 * Find all of the items which are instances of the class indicated by
 * the given fully-qualified className that are in the scope of the
 * given SpItem.
 */
public static Vector
findAllItems(SpItem rootItem, String className)
{
...
//
private static void
_findAllItems(SpItem rootItem, Class c, Vector v)
{
 ...
 * inserted, if legal.  Otherwise, if the item cannot be legally inserted
 * inside the parent, null is returned.
 */
public static SpInsertData
evalInsertInside(SpItem newItem, SpItem parent)
{
 ...
 */
public static SpInsertData
evalInsertInside(SpItem[] newItems, SpItem parent)
{
   // Make sure the parent isn't one of the newItems
   for (int i=0; i<newItems.length; ++i) {
      if (parent == newItems[i]) return null;
   }

   //// Make sure the parent isn't already the parent of the newItems
   //if (parent == newItems[0].parent()) return null;
...
//
static SpInsertInfo
doEvalInsertAfter(SpItem newItem, SpItem sibling)
{ 
   // Is there a definition for this combination?
 ...
 * @return A SpInsertData structure that describes where the newItems will be
 * inserted, if legal.  Otherwise, if the item cannot be legally inserted
 * after the sibling, null is returned.
 */
public static SpInsertData
evalInsertAfter(SpItem newItem, SpItem sibling)
{
   SpItem[] spItemA = { newItem };
 ...
 */
public static SpInsertData
evalInsertAfter(SpItem[] newItems, SpItem sibling)
{
   // Make sure the sibling isn't one of the newItems
   for (int i=0; i<newItems.length; ++i) {
      if (sibling == newItems[i]) return null;
   }
...
// 
private static SpInsertData
_completeEvalInsert(SpItem[] newItems, SpInsertInfo spII)
{
   if (spII == null) return null;
   ...
   for (int i=0; i<newItems.length; ++i) {
      if (referant == newItems[i]) return null;
   }
   ...
   for (int i=newItems.length-2; i>=0; --i) {
      ...
      if ((result != spII.result) || (referant != spII.referant)) return null;

      if (spII.replaceItem != null) {
         if (repV == null) repV = new Vector();
         repV.addElement(spII.replaceItem);
      }
   }
...
/**
 * Insert items into a program hierarchy according to the information
 * in the SpInsertData structure.
 *
 * <p>This method is typically called after one of the "eval" methods
 * has returned a non-null SpInsertData to use as the argument.
 */
public static void
insert(SpInsertData spID)
{
...
/**
 * Move the items in the program hierarchy according to the information
 * in the SpInsertData structure.
 *
 * <p>This method is typically called after one of the "eval" methods
 * has returned a non-null SpInsertData to use as the argument.
 ...
 */
public static boolean
evalExtract(SpItem[] spItems)
{
   for (int i=0; i<spItems.length; ++i) {
      if (!evalExtract(spItems[i])) return false;
   }
   return true;
}
 ...
 */
public static boolean
extract(SpItem spItem)
{
   if (!evalExtract(spItem)) return false;
   spItem.extract();
   return true;
}
 ...
 */
public static boolean
extract(SpItem[] spItems)
{
   if (!evalExtract(spItems)) return false;
