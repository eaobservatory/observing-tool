...
/**
 * Data that describes the insertion of a set of SpItems into or after
 * another SpItem.  The client uses the <code>result</code> together
 * with the <code>referant</code> item to know where the insertion would
 * take place.  For instance a result of INS_AFTER would mean that the
 * <code>items</code> would be inserted after the <code>referant</code>
 * item in the program.
 *
 * <p>
...
toString()
{
  String posn;
  switch (result) {
     case INS_INSIDE: posn = "INS_INSIDE";  break;
     case INS_AFTER:  posn = "INS_AFTER";   break;
     default: posn = "UNKNOWN (" + result + ")";
   }

   return getClass().getName() +
         "[" + posn + ", " + "(" + referant.getTitle() + ", " +
         referant.typeStr() + ", " + referant.subtypeStr() + "), " +
