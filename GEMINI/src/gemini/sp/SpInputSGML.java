...
final class SgmlStreamTokenizer extends StreamTokenizer
{
   ...
   /**
    * TT_TAG_START consists of < followed by a tag name.  sval is set
    * to the tag name.
    */
   ...
   /**
    * TT_TAG_END consists of <\ followed by a tag name, followed by >.
    * The sval variable is set to the tag name.
    */
...
SgmlStreamTokenizer(Reader r)
{
   ...
   //ordinaryChar('\n'); wordChars('\n', '\n');
   //ordinaryChar('\r'); wordChars('\r', '\r');

   ordinaryChar('\''); quoteChar('\'');
   ordinaryChar('"');  quoteChar('"');
   ordinaryChar('/');
   ordinaryChar('<');
   ordinaryChar('=');
...
/**
 * This is a simple recursive decent, top-down parser for Science Programs.
 * The grammar recognized is given below where <tt> tag </tt> stands for
 * any generic tag, <tt> e </tt> is the nil string, and <tt> id </tt> is
 * any identifier.
 *
 * <pre>
 *
...
/**
 * Constructs a parser for the SGML code for a Science Program in the
 * given InputStream.
 */
...
{
   if (token != _curToken) {
      _problem = "Expected "  + _tokenToString(token) +
                 ", but got " + _tokenToString(_curToken);
      return false;
   }
...
//
private void
_expected(int[] tokens, int actualToken)
{
...
_parseItem()
{
   ...
   if ((itemDef.name    == null) ||
       (itemDef.type    == null) ||
       (itemDef.subtype == null)    ) {
      _problem = "The item is missing a name, type, and/or subtype attribute.";
      return null;
   }

   // ---- temporary fix to read in old science programs with note components
   ...
   if (spType == null) {
      _problem = "Couldn't create an item of type '" + itemDef.type +
                 "' and subtype '" + itemDef.subtype + "'.";
      return null;
   }
...
//
private boolean
_parseDef(SpItemDef itemDef)
{
   //System.out.println("_parseDef(" + itemDef.toString() + ")");
...
//
private boolean
_parseContent(SpItem spItem)
{
   //System.out.println("_parseContent(" + spItem.typeStr() + ")");
...
//
private boolean
_parseAvDef(SpAttributeDef attrDef)
{   
   if (_curToken != StreamTokenizer.TT_WORD) {
...
//
private boolean
_parseValue(SpItem spItem, String attrName, int index)
{
...
//
private boolean
_parseValueList(SpItem spItem, String attrName, int index)
{
...
_parseChildren(SpItem spItem)
{
   ...
   while (_curToken == SgmlStreamTokenizer.TT_TAG_START) {
      ...
      if (childV == null) childV = new Vector();
   ...
   if ((spItem != null) && (spItem instanceof SpPhase1)) {
      ...
      if (_curToken == SgmlStreamTokenizer.TT_TAG_START) {
         ...
         if (!(spProgItem instanceof SpProg)) {
            _problem =  "Only items of type '" +
                         SpType.SCIENCE_PROGRAM.getReadable() +
                         "' may follow phase 1 items.\n";
            _problem += "Instead this file contained an item of type: " +
                                              spProgItem.type().getReadable();
            return null;
         }
...
/**
 * Parse the Science Program file, constructing the Java representation
 * and returning a reference to the root SpItem.
 */
public SpRootItem
parseDocument()
{
   ...
   if (spItem != null) {
      if (!(spItem instanceof SpRootItem)) {
          _problem = "Expected a top-level program item, but got: " +
                                                  spItem.type().getReadable();
          return null;
      }
...
{
   try {
      for (int tokenType = _st.nextToken();
           tokenType != StreamTokenizer.TT_EOF;
           tokenType = _st.nextToken()) {
