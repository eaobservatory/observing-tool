...
{
   return (_configItemAttr != null) && (_configItemAttr.size() > 0) &&
                                                    (_curStep < _totalSteps);
}
...
private SpIterValue[]
_getConfigValues(int stepIndex)
{
   String attr, val;
...
/**
 * The base class for iterators that iterate over configurations.
 * For example, the SpIterNIRI iterator iterates over filters, grisms,
 * etc.
 */
public abstract class SpIterConfigBase extends SpIterComp
{
   /**
    * The name of the attribute that lists the set of attribtes
    * being iterated over. For example, an iterator set up to
    * iterate over filters and integration time would have its
    * ATTR_ITER_ATTRIBUTES set to 
    * <pre> { filterIter, integrationTimeIter } </pre>.
    */
   public static final String ATTR_ITER_ATTRIBUTES = "iterConfigList";

   /**
    * The array of configuration items offered by this iterator.
    * This must be initialized by the subclass (and is pretty much the
...
/**
 * Get an array containing the IterConfigItems offered by this
 * iterator.
 */
...
getItemForAttribute(String attribute)
{
 ...
 */
public Vector
getConfigSteps(String attribute)
{
 ...
 */
public String
getConfigStep(String attribute, int index)
{
...
/**
 * Insert a new step at the given index. MODIFIED by AB to get
 * a default from the previous step (for index >0)
 */
public void
