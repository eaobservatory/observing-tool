...
nextElement()
{
  //  System.out.println ("In next element");
...
/**
 * This is the base class for enumerating the iteration steps of an
 * iterator.  It takes care of the details of stepping through
 * nested child iterators.  Subclasses should concentrate on providing
 * the implementation of the three protected abstract methods:
 *
 * <pre>
 ...
 * </pre>
 *
 * Each element returned by this Enumeration (i.e., the return value of
 * the nextElement() method) is a Vector of <b>SpIterStep</b>.  The
 * SpIterStep describes one step of the set of steps produced by an
 * iterator.  For instance, an instrument iterator might iterate over
 * filters and exposure time.  A single step of the iterator would set
 * an exposure time and a filter concurrently.
 * 
 * <p>
 * For each step of a parent of a nested iterator, the child iterators
 * must cycle through all of their values.  The Vector of SpIterSteps
 * returned by nextElement() is ordered from the outermost iterator to
 * the most deeply nested iterator.
 *
 * <h3>Example</h3>
 ...
 * </pre>
 * <p>
 * since the filter and offset have already been applied prior to the
 * first observe.  The next call produces:
 * <pre>
 *	Vector of SpIterStep
 ...
 * </pre>
 * <p>
 * Then in the next call, the filter is changed to H, the telescope
 * is offset to position 1, and an observe is applied.  This pattern continues
 * until all the values that can be produced are exhausted.
 *
 * <h3>Subclassing</h3>
 * Again, subclasses should concentrate on implementing only the three
 * protected abstract methods and not worry about nesting.  The combined
 * task of these three methods is to iterate over the steps contained in
 * a single iterator.  For instance, an Offset iterator should return
 * an SpIterStep describing the first offset position with the call to
 * _thisFirstElement().  Each subsequent call to _thisNextElement() should
 * return an SpIterStep the next offset position.  So long as there are
 * offset positions available, _thisHasMoreElements() should return true.
 */
...
/**
 * Construct the SpIterEnumeration with the iterator component that
 * it will be stepping through.
 */
 ...
 */
protected boolean    _thisHasCleanup() { return false; }

/**
 * This method has not come to its fruition.  No subclasses currently
 * override this method.
 */
protected SpIterStep _thisCleanup()    { return null;  }

/**
 * Returns true if there are more steps before the enumeration is
 * completely exhausted.  This method is used by clients of this
 * class and need not be overriden.  See the discussion in the 
