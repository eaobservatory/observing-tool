...
/**
 * This class maintains a table of attributes and their values.  The table
 * is a simple hashtable in which the keys are the attribute names and the
 * values are instances of SpAttr (a description, vector of strings pair).
 * Since a vector is stored in each SpAttr, multiple values are permitted
 * for each attribute.
 */
public final class SpAvTable implements java.io.Serializable
{
   private static final int   AV_TAB_INIT_SIZE   = 12;
...
//
void
setStateMachine(SpAvEditState editFSM)
{
   _editFSM = editFSM;
}
...
/**
 * Set the description of an attribute without notifying any observers.
 * This method is called when initializing the AV table so that observers
 * aren't sent a bunch of spurious changes.
 *
 * @see #setDescription
 */
public void
noNotifySetDescription(String name, String descr)
{
 ...
 */
public void
setDescription(String name, String descr)
{
   if (_editFSM != null) {
 ...
 */
public void
noNotifySet(String name, String value, int pos)
{
   SpAttr a = _getSpAttr(name);
 ...
 */
public void
set(String name, String value, int pos)
{
 ...
 */
public void
noNotifySetAll(String name, Vector v)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) {
 ...
 */
public void 
set(String name, boolean value, int pos)
{
   if (value) {
      set(name, "true", pos);
   } else {
...
/**
 * Set the value of a particular attribute at a particular position as
 * an int.
 */
public void 
set(String name, int value, int pos)
{
 ...
 */
public void 
set(String name, double value, int pos)
{
   set(name, String.valueOf(value), pos);
}
...
/**
 * Get the value of a particular attribute at a particular position
 * as a string.
 */
public String
get(String name, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) {
      return null;
...
/**
 * Get the value of a particular attribute at a particular position as
 * an int.
 */
public int
getInt(String name, int pos, int def)
{
   String value = get(name, pos);
   if (value == null) {
      return def;
 ...
 */
public double
getDouble(String name, int pos, double def)
{
   String value = get(name, pos);
   if (value == null) {
      return def;
...
_rmValue(String name, int pos)
{
   ...
   // If there are no more values for this attribute, remove the attribute
   // all together
   //if (v.size() == 0) {
   //   _avTable.remove(name);
   //}
...
/**
 * Get an enumeration of the attributes that start with the given
 * prefix.
 */
public Enumeration
...
//
private void
_setSize(String name, int newSize)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
 ...
 * current size, empty strings are appended for the missing elements.  If
 * the new size is less than the current size, all the elements at index
 * newSize and greater are discarded.
 */
public void
setSize(String name, int newSize)
{
   if (_isHiddenAttr(name) || (_editFSM == null)) {
...
//
private void
_insertAt(String value, Vector v, int pos)
{
   if (pos >= v.size()) {
      v.addElement(value);
   } else {
 ...
 */
public void
insertAt(String name, String value, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
...
//
private void
_indexToFirst(Vector v, int pos)
{
...
/**
 * Move the element of the named attribute at the given position to
 * the front, or zeroth position.
 */
public void
indexToFirst(String name, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
...
//
private void
_decrementIndex(Vector v, int pos)
{
   Object o = v.elementAt(pos);
   v.removeElementAt(pos);
...
/**
 * Move the element of the named attributed at the given position
 * up one (toward index 0).
 */
public void
decrementIndex(String name, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
...
//
private void
_incrementIndex(Vector v, int pos)
{
   Object o = v.elementAt(pos);
   v.removeElementAt(pos);
   if ((pos + 1) >= v.size()) {
 ...
 */
public void
incrementIndex(String name, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
...
/**
 * Move the element of the named attributed at the given position to
 * the last position.
 */
public void
indexToLast(String name, int pos)
{
   SpAttr a = (SpAttr) _avTable.get(name);
   if (a == null) return;
